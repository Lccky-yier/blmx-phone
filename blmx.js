...
`;return a.trim()}(q.id);O=a;let i=await v({user_input:a,should_stream:!1});M=i.trim();let n=document.getElementById("chat-simulation-log");n.innerHTML="";let l=i.trim().split("\n");for(let r of l){if(!r)continue;let o=/UPDATE_CALL_SCREEN:({.*})/,s=r.match(o);if(s)eu(JSON.parse(s[1]));else{let d=document.createElement("div");d.className="chat-simulation-message",d.textContent=r,n.insertBefore(d,n.firstChild)}}}catch(c){console.error("[Call Accept] AI failed to respond after call accept:",c)}finally{b=!1,eX(),updateInCallButtonUI()}}function eh(){console.log("[Call] User declined the call.");let e=[...w.logEntries].reverse().find(e=>"VIDEO_CALL"===e.key&&e.data.caller===q.id);e&&(e.data.status="declined",w.persistLogToStorage());let t=C.find(e=>"single"===e.type&&e.members.includes(q.id));if(t){let a={convoId:t.id,timestamp:new Date(window.currentGameDate).toISOString().slice(0,16).replace("T"," "),description:`你拒接了 ${q.name} 的视频通话邀请。`};w.addEntry({type:"event_log",content:a}),w.persistLogToStorage(),eS(t.id,a.timestamp)}P="idle",eg(null)}function eb(){ev();let e=document.getElementById("call-timer");e&&(R=0,e.textContent="00:00",H=setInterval(()=>{R++;let t=Math.floor(R/60).toString().padStart(2,"0"),a=(R%60).toString().padStart(2,"0");e.textContent=`${t}:${a}`},1e3))}function ev(){clearInterval(H),H=null}function ew(e){let t=_.find(t=>t.id===e);if(!t)return;document.getElementById("contact-details-profile-card").dataset.contactId=t.id,document.getElementById("contact-details-avatar").src=ec(t.id),document.getElementById("contact-details-name").textContent=ed(t.id,null),document.getElementById("contact-details-avatar").dataset.contactId=t.id;let a=document.getElementById("set-private-chat-wallpaper-btn"),i=C.find(e=>"single"===e.type&&e.members.includes(t.id));i?(a.style.display="flex",a.dataset.convoId=i.id):a.style.display="none";let n=document.querySelector("#block-contact-btn .item-label");n&&(n.textContent=t.isBlocked?"解除拉黑":"加入黑名单")}function eE(e){let t=document.getElementById("diary-entry-view"),a=document.getElementById("diary-entry-title-h1"),i=document.getElementById("diary-entry-content-div"),n=document.getElementById("diary-entry-meta-p"),l=document.getElementById("diary-header-avatar");if(a.textContent="",i.innerHTML="",n.innerHTML="",l.style.display="none",null===e){a.textContent="空空如也",i.innerHTML="<p>这里还没有任何日记。点击右上角的“+”号来写下第一篇吧！</p>",t.dataset.viewingIndex="-1";let r=t.dataset.ownerId;r&&(l.src=ec(r),l.style.display="block"),eI();return}if(w&&"number"==typeof e&&w.logEntries[e]){let o=w.logEntries[e];if("DIARY_ENTRY"===o.key){let s=o.data;if(a.textContent=s.title||"无标题日记",n.innerHTML=`
        ...
        `;let r=n.querySelector(".recall-notice-text"),o=n.querySelector(".recall-content");r.addEventListener("click",()=>{o.classList.toggle("expanded")}),t?t.replaceWith(n):c.appendChild(n)}function e$(e){if(c.innerHTML="",!w)return;let t=w.logEntries.map((e,t)=>({...e,originalIndex:t})).filter(t=>{let a=t.convoId||t.conversationId||t.content&&t.content.convoId||t.data&&t.data.convoId;return a===e}),a=E.filter(t=>t.conversationId===e).map((e,t)=>({...e,originalIndex:w.logEntries.length+t,timestamp:new Date().toISOString()})),i=[...t,...a];i.sort((e,t)=>{if(void 0!==e.originalIndex&&void 0!==t.originalIndex)return e.originalIndex-t.originalIndex;let a=e=>e.timestamp?new Date(e.timestamp):e.data&&e.data.timestamp?new Date(e.data.timestamp):e.content&&e.content.timestamp?new Date(e.content.timestamp):null,i=a(e),n=a(t);return i&&n?i-n:0}),i.forEach(e=>{"RECALL_MESSAGE"!==e.key&&function e(t,a){if("RECALL_MESSAGE"===t.key){let i=t.data.target_text,n=t.data.author,r=Array.from(c.querySelectorAll(".message-row")).reverse(),o=r.find(e=>{let t=e.querySelector(".message-bubble");if(!t)return!1;let a=e.querySelector(".message-avatar").dataset.senderId;return a===n&&t.textContent.trim()===i});o&&e9(t,o,a);return}switch(t.type){case"event_log":e8(t.content,a);break;case"group_event":ek(t.content,a);break;case"time":break;case void 0:t.key||console.warn("Undefined entry type, skipping UI add:",t);break;default:(function e(t,a){let{id:i,sender:n,type:r,data:o}=t,s="user"===n||"{{user}}"===n||n===A.name?"me":"them",d=t.convoId||t.conversationId,m=C.find(e=>e.id===d),p=document.createElement("div");p.className="message-row "+s,void 0!==a&&(p.dataset.logIndex=a),p.dataset.messageId=i;let u=m&&"vgroup"===m.type&&!m.members.includes(n),g="me"===s?"user":n,y;if(u){if(U.has(n))y=U.get(n);else{let f=new Set(U.values()),h=X.filter(e=>!f.has(e)),b=h.length>0?h[Math.floor(Math.random()*h.length)]:X[Math.floor(Math.random()*X.length)];U.set(n,b),y=b}}else y=ec(g);let v=`<img src="${y}" class="message-avatar" data-sender-id="${g}">`,x=document.createElement("div");if(x.className="message-content-wrapper",m&&("group"===m.type||"vgroup"===m.type)&&"them"===s){let $=document.createElement("div");$.className="sender-name-label",$.textContent=u?n:ed(n,d),x.appendChild($)}let L="",B="message-bubble";switch(t.isFailed&&(B+=" failed-message-bubble"),r){case"sticker":let D=function e(t){let a=[];a.push(...ea,...JSON.parse(localStorage.getItem(et)||"[]")),_.forEach(e=>{let t=JSON.parse(localStorage.getItem(ei(e.id))||"[]");a.push(...t)}),C.forEach(e=>{if("group"===e.type){let t=JSON.parse(localStorage.getItem(ei(e.id))||"[]");a.push(...t)}else if("single"===e.type){let i=e.members.find(e=>"user"!==e);if(i){let n=JSON.parse(localStorage.getItem(ei(i))||"[]");a.push(...n)}}});let i=a.filter((e,t,a)=>e.label&&t===a.findIndex(t=>t.label===e.label)),n=i.find(e=>e.label===t);return n?n.url:void 0}(t.content);D?(L=`<img src="${D}" alt="${t.content}">`,B+=" sticker-bubble"):L=`[表情: ${t.content}]`;break;case"forward":{let M=t.data,O="转发的内容",P=M.messageIds,H=!1;if(P&&P.length>0){let R=P[0];if(w.logEntries.some(e=>"MOMENT"===e.key&&e.data.momentId===R))H=!0,O="转发的动态";else{let q=!1;try{let G=JSON.parse(R);G&&"weibo_post"===G.type&&(q=!0)}catch(W){}O=q?"转发的帖子":"转发的聊天记录"}}let j="",Y=null;if(P&&P.length>0){let z=P[0];if(H){let F=w.logEntries.find(e=>"MOMENT"===e.key&&e.data.momentId===z);if(F){let V=F.data,K=ed(V.author,null),J=V.text||"[图片动态]";j=`<p style="display: flex; align-items: center; gap: 5px;"><i class="far fa-images" style="color: #4CAF50;"></i><strong>@${K}</strong></p><p>${J.substring(0,30)}</p>`}else j=`<p>一条已失效的朋友圈动态</p>`}else{let Z=!1;try{let Q=JSON.parse(z);Q&&"weibo_post"===Q.type&&(Z=!0,Y=Q.postId,j=`<p style="display: flex; align-items: center; gap: 5px;"><i class="fab fa-weibo" style="color: #E14438;"></i><strong>@${Q.author}</strong></p><p>${Q.title||Q.summary}</p>`)}catch(ee){}if(!Z){let en=M.messageIds.map(e=>w.logEntries.find(t=>t.id===e)).filter(Boolean);en.slice(0,2).forEach(e=>{let t=`[${e.type||"复合消息"}]`;e.content&&("string"==typeof e.content?t=e.content:e.content.text&&(t=e.content.text)),j+=`<p>${ed(e.senderId||e.sender,null)}: ${t.substring(0,20)}</p>`})}}}let el=Y?`data-post-id="${Y}"`:"";L=`<div class="forward-card" data-forward-id="${i}" data-message-ids='${JSON.stringify(M.messageIds)}' ${el}>
							<div class="forward-title">${O}</div>
							<div class="forward-summary">${j}</div>
						</div>`,B+=" forward-bubble";break}case"image":if(t.content&&"url"===t.content.type){let er=t.content.value;if(er.startsWith("blmx-img-")){let eo=sessionStorage.getItem(er);er=eo||"https://files.catbox.moe/bialj8.jpeg"}L=`<img src="${er}" alt="图片">`,B+=" image-url-bubble"}else{let es=t.content&&t.content.value?t.content.value:t.content,em=l(t.id,es);em?(L=em,em.includes("<img")?B+=" sticker-bubble":B+=" image-desc-bubble"):(L=`<div class="image-desc-content">${String(es).replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div>`,B+=" image-desc-bubble")}break;case"location":L=`<div class="location-card"><div class="location-content"><div class="location-title">${t.content}</div><div class="location-subtitle">共享实时位置</div></div><div class="location-map-placeholder"></div></div>`,B+=" location-bubble";break;case"transfer":{let eu="string"==typeof o?JSON.parse(o):o,eg="sent"!==eu.status,ey="";m&&"group"===m.type&&eu.recipientId&&(ey=`<div class="recipient-note">转账给：${ed(eu.recipientId,d)}</div>`);let ef=eg?`<div class="status-text">${"accepted"===eu.status?"已接收":"已退还"}</div>`:`${ey}<div class="note">${eu.note||" "}</div>`;L=`<div class="transfer-card ${eg?"transfer-receipt":"transfer-initial"}"><div class="transfer-content"><img src="https://files.catbox.moe/y8059q.png" class="transfer-icon-image"><div class="transfer-details"><div class="amount">\xa5${eu.amount}</div>${ef}</div></div><div class="transfer-footer">转账</div></div>`,B+=" transfer-bubble";break}case"file":L=`<div class="file-card"><div class="file-content"><i class="fas fa-file-alt file-icon"></i><div class="file-details"><div class="file-name">${t.content.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div></div></div><div class="file-footer">文件</div></div>`,B+=" file-bubble";break;case"gift":{let eh="string"==typeof o?JSON.parse(o):o,eb="";m&&"group"===m.type&&eh.recipientId&&(eb=`<div class="recipient-note">送给：${ed(eh.recipientId,d)}</div>`);let ev="";L=`<div class="gift-content"><i class="fas fa-gift gift-icon"></i><div class="gift-details"><div class="gift-name">${eh.name}</div>${ev="sent"===eh.status?`${eb}`+(eh.price?`<div class="gift-price">\xa5 ${eh.price}</div>`:""):`<div class="gift-status-text">${"accepted"===eh.status?"已收下":"已拒收"}</div>`}</div></div><div class="gift-footer">礼物</div>`,B+=" gift-bubble","sent"!==eh.status&&(B+=" gift-receipt");break}case"music_share":{let ew=t.data||{},eE=ew.cover;eE&&"https://files.catbox.moe/g3x1v8.jpg"!==eE||(eE=ec(t.sender)),L=`
							<div class="music-share-card" data-song-info='${JSON.stringify(ew)}'>
								<div class="music-share-cover" style="background-image: url('${eE}');"></div>
								<div class="music-share-info">
									<div class="music-share-title">${ew.title||"未知歌曲"}</div>
									<div class="music-share-artist">${ew.artist||"未知歌手"}</div>
								</div>
							</div>
							<div class="music-share-footer">
								<i class="fas fa-music"></i> 一起听
							</div>
						`,B+=" music-share-bubble";break}case"payment_receipt":let eI={};try{"object"==typeof t.content&&null!==t.content?eI=t.content:"string"==typeof t.content&&(eI=JSON.parse(t.content))}catch(e9){console.error("代付数据解析失败",e9),eI={total:"0.00",payer:"未知",receiver:"未知"}}let e8="",eL=eI.items||[];eL.slice(0,3).forEach(e=>{e8+=`
            ...
            </div>`}),eL.length>3&&(e8+=`<div class="receipt-row" style="color:#bbb; font-size:0.8em;">... 等共 ${eL.length} 件商品</div>`);let ek=eI.timestamp?eI.timestamp:eT(t.timestamp);L=`
    ...
    `,B="message-bubble shopping-receipt-bubble";break;case"product_share":let eS=t.content,eD=eS.image;!eD||eD.startsWith("http")||eD.startsWith("blob:")||(eD="https://files.catbox.moe/c41va3.jpg"),L=`
                    <div class="product-share-card" data-product='${JSON.stringify(eS)}'>
                        <div class="ps-content">
                            <img src="${eD}" class="ps-img">
                            <div class="ps-info">
...
<div class="red-packet-card">
	...
	<div class="red-packet-footer">微信红包</div>
					...
					`,B+=" voice-bubble";break;default:let eM=t.content.match(/^\[引用:\s*"(.*?):\s*(.*?)"\]\s*(.*)$/s);if(eM){let eO=eM[1],eN=eM[2],e0=eM[3],eP=eN.match(/^\[语音:\s*(.*)\]$/);if(eP)try{let eH=eP[1].replace(/\\"/g,'"'),eR=JSON.parse(eH);eN=`语音：${eR.text}`}catch(eq){console.error("解析引用的语音消息失败:",eq,"原始字符串:",eP[1]),eN="[语音]"}else eN.startsWith("[表情:")?eN="[表情]":eN.startsWith("[图片:")?eN="[图片]":eN.startsWith("[位置:")&&(eN="[位置]");L=`
								<div class="quote-container">
									<div class="quote-author">${eO}</div>
									<div class="quote-content">${eN.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div>
								</div>
								<span class="reply-text">${e0.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</span>
							`}else L=t.content.replace(/</g,"&lt;").replace(/>/g,"&gt;")}let e3=document.createElement("div");e3.className=B,e3.innerHTML=L,x.appendChild(e3);let eU=document.createElement("input");eU.type="checkbox",eU.className="forward-checkbox",eU.dataset.messageId=i;let eG=null;if(t.isFailed&&((eG=document.createElement("i")).className="fas fa-exclamation-circle failed-status-icon"),"me"===s?(p.appendChild(eU),eG&&p.appendChild(eG),p.appendChild(x),p.insertAdjacentHTML("beforeend",v)):(p.appendChild(eU),p.insertAdjacentHTML("beforeend",v),p.appendChild(x),eG&&p.appendChild(eG)),"voice"===r&&e3.addEventListener("click",()=>{let e=e3.querySelector(".voice-text-content");e&&e.classList.toggle("expanded")}),"forward"===r){let eW=e3.querySelector(".forward-card");eW.addEventListener("click",()=>{let e=eW.dataset.postId;if(e){let t=N.posts.find(t=>t.postId===e);t?(ep("weiboDetail",{postId:e,category:t.category}),eA(e)):showDialog({mode:"alert",text:"该微博已被作者删除或链接已失效。"})}else{let a=eW.dataset.messageIds,i=JSON.parse(a),n=!1;try{let l=JSON.parse(i[0]);l&&"weibo_post"===l.type&&(n=!0)}catch(r){}if(n)showDialog({mode:"alert",text:"这是一个微博转发预览，但由于格式原因无法直接跳转。"});else if(i[0].startsWith("moment_"))(function e(t,a){let i=document.getElementById("forward-content-modal"),n=document.getElementById("forward-content-modal-body"),l=i.querySelector(".title"),r=parseInt(t.replace("moment_",""),10),o=w.logEntries[r];if(l.textContent="查看动态",n.innerHTML="",!o||"MOMENT"!==o.key){n.innerHTML=`
...
`,r.innerHTML="";let p=N.posts.filter(e=>e.author===d.id||c&&"{{user}}"===e.author).sort((e,t)=>new Date(t.timestamp)-new Date(e.timestamp));p.length>0?p.forEach(e=>{let t=document.createElement("div");t.className="post-card",t.dataset.postId=e.postId;let a="",i=e.title||"无标题帖子",n=i.match(/^(?:\[|【)([^\]】]+)(?:\]|】)\s*(.*)$/);if(n){let l=n[1],o=n[2];a=`
		...
		`,o.appendChild(y)}let f=document.createElement("div");if(f.className="ama-qna-list",o.appendChild(f),w){let h=w.logEntries.filter(t=>"AMA_PAIR"===t.key&&(t.data.author===e||c&&"{{user}}"===t.data.author)).sort((e,t)=>new Date(t.data.timestamp)-new Date(e.data.timestamp));if(u&&h.length>0){let b=o.querySelector("p");b&&(b.style.display="none")}h.forEach(e=>{let t=e.data,a=document.createElement("div");a.className="qna-card",a.innerHTML=`
			...
			`,R="",q=/#([^#\s]+)/g,U=[...(t.text||"").matchAll(q)].map(e=>e[1]);if(U.length>0){let G=[...new Set(U)];R=`
				...
				`,g=g.replace(q,"").trim()}let W=(N.comments[e]||[]).length,j=`
