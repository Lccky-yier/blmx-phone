		...
		async function parseAndHandleAiResponse(rawResponse, context = {}) {
			...
			for (const line of lines) {
				...
				if (commandMatch) {
					...
					const convoId = chatMatch[1];
					const senderId = chatMatch[2].trim();
					let content = chatMatch[3].trim();
					
					// --- [核心修复] 针对 "内容里包着指令" 的情况 ---
					// 对应你说的情况1: [ID] Name: [指令]
					
					// 1. 剥皮：去掉内容外层可能存在的方括号 [ ]
					// 使用正则替换，比 slice 更安全，能处理带空格的情况 [ 指令 ]
					let cleanContent = content.replace(/^\[\s*|\s*\]$/g, '');
					
					// 2. 检查是否符合 "指令:JSON" 格式
					const nestedCommandRegex = new RegExp(`^(${commandKeywords.join('|')}):\\s*(\\{.*\\})$`);
					const nestedMatch = cleanContent.match(nestedCommandRegex);
					
					if (nestedMatch) {
						console.log(`[BLMX Parser] 拦截到聊天内嵌指令: ${nestedMatch[1]}`);
						// 提取出指令和数据，转交给指令处理器，不再作为消息发送
						processCommandData(nestedMatch[1], nestedMatch[2]);
					} else {
						// 3. 如果剥皮后还不是指令，那它就是真消息
						handleAiChatMessage(content, senderId, convoId);
					}
				}
				else {
					// === 情况 C: 格式错乱的救急匹配 ===
					// 对应你说的情况2: [ID] [指令...] (缺少 Name 和冒号)
					// 或者任何 AI 抽风把指令藏在奇怪符号里的情况
					
					// 正则含义：只要行内包含 "关键词:{...}" 这种结构，就抓出来
