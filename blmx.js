...
`;return n.trim()}(q.id);N=i;let n=await v({user_input:i,should_stream:!1});M=n.trim();let l=document.getElementById("chat-simulation-log");l.innerHTML="";let r=n.trim().split("\n");for(let o of r){if(!o)continue;let s=/UPDATE_CALL_SCREEN:({.*})/,d=o.match(s);if(d)try{eu(JSON.parse(d[1]))}catch(c){console.error("解析画面指令失败:",c)}else{let m=document.createElement("div");m.className="chat-simulation-message",m.textContent=o,l.insertBefore(m,l.firstChild)}}}catch(p){console.error("[Call Accept] AI failed to respond:",p);let u=document.getElementById("chat-simulation-log"),g=document.createElement("div");g.className="chat-simulation-message",g.style.color="#ff9999",g.textContent="(信号连接不稳定...)",u.insertBefore(g,u.firstChild)}finally{b=!1,eX(),"function"==typeof updateInCallButtonUI&&updateInCallButtonUI()}}function eh(){console.log("[Call] User declined the call.");let e=[...w.logEntries].reverse().find(e=>"VIDEO_CALL"===e.key&&e.data.caller===q.id);e&&(e.data.status="declined",w.persistLogToStorage());let t=C.find(e=>"single"===e.type&&e.members.includes(q.id));if(t){let a={convoId:t.id,timestamp:new Date(window.currentGameDate).toISOString().slice(0,16).replace("T"," "),description:`你拒接了 ${q.name} 的视频通话邀请。`};w.addEntry({type:"event_log",content:a}),w.persistLogToStorage(),eS(t.id,a.timestamp)}P="idle",eg(null)}function eb(){ev();let e=document.getElementById("call-timer");e&&(R=0,e.textContent="00:00",H=setInterval(()=>{R++;let t=Math.floor(R/60).toString().padStart(2,"0"),a=(R%60).toString().padStart(2,"0");e.textContent=`${t}:${a}`},1e3))}function ev(){clearInterval(H),H=null}function ew(e){let t=D.find(t=>t.id===e);if(!t)return;document.getElementById("contact-details-profile-card").dataset.contactId=t.id,document.getElementById("contact-details-avatar").src=ec(t.id),document.getElementById("contact-details-name").textContent=ed(t.id,null),document.getElementById("contact-details-avatar").dataset.contactId=t.id;let a=document.getElementById("set-private-chat-wallpaper-btn"),i=C.find(e=>"single"===e.type&&e.members.includes(t.id));i?(a.style.display="flex",a.dataset.convoId=i.id):a.style.display="none";let n=document.querySelector("#block-contact-btn .item-label");n&&(n.textContent=t.isBlocked?"解除拉黑":"加入黑名单")}function eE(e){let t=document.getElementById("diary-entry-view"),a=document.getElementById("diary-entry-title-h1"),i=document.getElementById("diary-entry-content-div"),n=document.getElementById("diary-entry-meta-p"),l=document.getElementById("diary-header-avatar");if(a.textContent="",i.innerHTML="",n.innerHTML="",l.style.display="none",null===e){a.textContent="空空如也",i.innerHTML="<p>这里还没有任何日记。点击右上角的“+”号来写下第一篇吧！</p>",t.dataset.viewingIndex="-1";let r=t.dataset.ownerId;r&&(l.src=ec(r),l.style.display="block"),eI();return}if(w&&"number"==typeof e&&w.logEntries[e]){let o=w.logEntries[e];if("DIARY_ENTRY"===o.key){let s=o.data;if(a.textContent=s.title||"无标题日记",n.innerHTML=`
        ...
        <div class="forward-summary">${j}</div>
						...
						`,B+=" music-share-bubble";break}case"payment_receipt":let eE={};try{"object"==typeof t.content&&null!==t.content?eE=t.content:"string"==typeof t.content&&(eE=JSON.parse(t.content))}catch(eI){console.error("代付数据解析失败",eI),eE={total:"0.00",payer:"未知",receiver:"未知"}}let e9="",e$=eE.items||[];e$.slice(0,3).forEach(e=>{e9+=`
            ...
            </div>`}),e$.length>3&&(e9+=`<div class="receipt-row" style="color:#bbb; font-size:0.8em;">... 等共 ${e$.length} 件商品</div>`);let eL=eE.timestamp?eE.timestamp:eT(t.timestamp);L=`
...
`,i};r.forEach(e=>t.appendChild(m(e,"pinned"))),s.forEach(e=>t.appendChild(m(e,"hot"))),c.forEach(e=>t.appendChild(m(e,"")))}function eD(e){let t=document.getElementById("weibo-detail-view"),a=t.querySelector(".weibo-detail-body"),i=e.postId||e.target_post_id,n=O.posts.find(e=>e.postId===i);if(!n)return;let r=n.author,o=e.author.endsWith("BOT");if(o){let s=document.createElement("div");s.className="forum-bot-reply",s.innerHTML=`
        ...
        `,s.appendChild(h)}let b=document.createElement("div");if(b.className="ama-qna-list",s.appendChild(b),w){let v=w.logEntries.filter(t=>"AMA_PAIR"===t.key&&(t.data.author===e||c&&"{{user}}"===t.data.author)).sort((e,t)=>new Date(t.data.timestamp)-new Date(e.data.timestamp));if(y&&v.length>0){let E=s.querySelector("p");E&&(E.style.display="none")}v.forEach(e=>{let t=e.data,a=document.createElement("div");a.className="qna-card",a.innerHTML=`
			...
			`,R="",q=/#([^#\s]+)/g,U=[...(t.text||"").matchAll(q)].map(e=>e[1]);if(U.length>0){let G=[...new Set(U)];R=`
				...
				</div>`;_.innerHTML=M+H+j}else{let Y=t.author.endsWith("BOT");if(Y){let z=document.createElement("div");return z.className="forum-bot-reply",z.innerHTML=`
						<i class="fas fa-robot bot-icon"></i>
						<div class="bot-content-wrapper">
...
2. **Formatting:** The "content" must be **plain text only**. Do NOT use any HTML tags like <p>, <del>, etc. Use newline characters (\\n) to separate paragraphs.
		...
		`;return i.trim()}(e);if(!t){await showDialog({mode:"alert",text:"无法生成AI指令，请检查角色是否存在。"}),b=!1,eX();return}N=t;try{let a=await v({user_input:t,should_stream:!1});M=a,a.trim()?(await ao(a),await showDialog({mode:"alert",text:`${ed(e,null)} 的新日记已写好！`})):await showDialog({mode:"alert",text:"AI未能生成有效的日记内容。"})}catch(i){console.error(`[日记功能] AI生成日记失败:`,i),await showDialog({mode:"alert",text:`AI响应失败: ${i.message}`})}finally{b=!1,eX(),document.getElementById("diary-entry-view").classList.contains("active")&&ep("diaryEntry",{ownerId:e})}}async function eF(){if(b){await showDialog({mode:"alert",text:"AI正在思考中，请稍后再试。"});return}let e=await showDialog({mode:"confirm",text:"是否要让时间流逝，看看角色们发生了什么？\n(这将消耗一次API调用来生成全局动态)"});if(e){b=!0,eX(),await showDialog({mode:"alert",text:"正在捕捉世界动态，请稍候..."});try{let t=function e(){let t=au(),a=t.map(e=>`- 【${e.title}】(ID: ${e.id})`).join("\n"),i=`
[任务：世界动态导演]
...
`,b=!0,eX();try{let a=function e(t){D.find(e=>e.id===t);let a=ed(t,null),i=new Date(window.currentGameDate),n=`${i.getMonth()+1}月${i.getDate()}日`;return`
[任务: 生成角色行动轨迹与位置状态]
角色: ${a} (ID: ${t})。
请生成该角色在今日的足迹数据。这些数据将成为该角色当前的“物理位置锚点”。

【要求】
1. 只输出一条 JSON 指令。
2. \`items\`: 包含 4-5 个节点。
3. \`duration\`: **必须生成**。描述在该地停留了多久（如：1小时30分，20分钟）。
4. **\`event\` (核心字段)**: 描写角色在该地点正在做的动作、环境氛围或突发小事（30字以内）。
*示例*: "正靠在靠窗的位子发呆，面前的拿铁已经凉了，窗外在下雨。"
5. \`current_location\`: 该角色“当前”最有可能停留的位置。

【输出格式】
FOOTPRINTS:{"author":"${t}","date":"${n}","current_location":"目前位置","items":[{"time":"08:30 - 10:00","location":"地点名","title":"活动名","duration":"1小时30分","event":"具体的心理或事件描述"}]}
...
`;let n=document.getElementById("manual-gen-footprints-btn");n&&n.addEventListener("click",()=>t2(t))}}();else if("gallery"===a){let i=B;if(!i)return;ep("gallery"),t4(i)}else if("shopping"===a){let n=B;if(!n)return;ep("shoppingHome")}else{let l=t.querySelector(".cp-app-name").textContent;showDialog({mode:"alert",text:`正在打开 [${l}] ...
(功能开发中)`})}}});let r=document.getElementById("shopping-home-feed");r&&r.addEventListener("click",e=>{let t=e.target.closest(".tb-product-card");if(!t||e.target.classList.contains("tb-img-placeholder-text"))return;let a=t.querySelector("img"),i=a?a.src:null,n=t.querySelector(".tb-img-placeholder-text"),l={title:t.querySelector(".tb-product-title").textContent,price:t.querySelector(".tb-price-num").textContent,shop:t.querySelector(".tb-sales").textContent,image:n?n.dataset.fullText:a?a.src:"",_imgBlobUrl:i};ep("productDetail"),ar(l)}),document.getElementById("tb-detail-back-btn").addEventListener("click",()=>{ep("shoppingHome")}),document.getElementById("tb-detail-add-cart").addEventListener("click",()=>{tQ()}),document.getElementById("tb-detail-buy-now").addEventListener("click",async()=>{if(!L)return;let e=B;if(!T&&e){let t=C.find(t=>"single"===t.type&&t.members.includes(e));t||(t={id:`convo_single_${e}`,type:"single",members:["user",e],unread:0,pinned:!1,lastActivity:Date.now()},C.push(t)),T=t.id}if(!T){await showDialog({mode:"alert",text:"错误：无法确定分享目标，请先进入一个聊天窗口或选择查看对象的手机。"});return}let a=await showDialog({mode:"confirm",text:"确定要立即购买并分享给对方吗？"});if(a){let i={title:L.title,price:L.price,image:L.image,shop:L.shop};e1({type:"product_share",sender:"me",content:i}),await showDialog({mode:"alert",text:"商品已分享！"}),ep("wechatChat",{conversationId:T})}});let o=document.querySelector(".tb-clear-home-btn");o&&o.addEventListener("click",async()=>{let e=B;if(!e)return;let t=await showDialog({mode:"confirm",text:"确定要清空当前的商品推荐流吗？\n清空后可点击刷新按钮重新生成。"});t&&(w.logEntries=w.logEntries.filter(t=>!("taobao_home"===t.type&&t.author===e)),await w.persistLogToStorage(),an(e),await showDialog({mode:"alert",text:"推荐流已清空。"}))}),document.getElementById("cp-shopping-manage-btn").addEventListener("click",()=>{ae()});let s=document.getElementById("nav-to-home-from-cart");s&&s.addEventListener("click",()=>{ep("shoppingHome")});let g=document.getElementById("nav-to-cart");g&&g.addEventListener("click",()=>{let e=B;e&&(ep("shopping"),at(e))}),document.getElementById("cp-shopping-back-btn"),document.querySelector(".tb-scan-icon").addEventListener("click",()=>{ep("checkPhone")}),document.getElementById("cp-shopping-back-btn").addEventListener("click",()=>{ep("shoppingHome")});let y={想去旅行:{name:"想去旅行",instruction:"推荐旅行相关的商品，如机票、酒店套餐、旅行收纳、度假穿搭。根据角色最近是否劳累决定是推荐'特种兵旅游'还是'躺平度假'。"},深夜冲动:{name:"深夜冲动",instruction:"推荐适合深夜下单的商品：高热量夜宵、成人玩具、昂贵的数码产品、情感抚慰用品。侧重于'冲动消费'和'私密欲望'。"},闲置回血:{name:"闲置回血",instruction:"【特殊反向逻辑】推荐角色**想要卖掉**的二手物品。例如：前任送的礼物、不再喜欢的旧物、冲动消费后的后悔药。店铺名显示为'我的闲置'。"},海外代购:{name:"海外代购",instruction:"推荐昂贵的进口商品、奢侈品、或者国内买不到的小众好物。"},健康养生:{name:"健康养生",instruction:"推荐保健品、养生壶、护肝片、防脱发产品。侧重于当代年轻人的'赛博养生'。"},礼物推荐:{name:"礼物推荐",instruction:"角色可能正在考虑给某人（可能是{{user}}，也可能是其他人）买礼物。推荐一些具有送礼属性的精致商品。"},角色穿搭:{name:"角色穿搭",instruction:"基于角色的外貌描写和风格，推荐TA可能会喜欢的衣服、配饰、鞋包。"},宅家必备:{name:"宅家必备",instruction:"推荐提升居家幸福感的好物，游戏机、懒人沙发、零食大礼包、智能家居。"},浏览历史:{name:"浏览历史",instruction:"展示角色最近看过但没买的东西，反映TA的纠结和犹豫。"},全部频道:{name:"猜你喜欢",instruction:"生成一组完全随机、脑洞大开的商品，试图探测角色的潜在兴趣。"}},f=document.querySelector(".tb-icons-grid");f&&f.addEventListener("click",async e=>{let t=e.target.closest(".tb-icon-item");if(!t)return;let a=t.querySelector(".tb-icon-label").textContent.trim(),i=y[a],n=B;if(!n||!i)return;let l=await showDialog({mode:"confirm",text:`进入【${a}】频道？
这会刷新当前的商品推荐流。`});l&&ai(n,null,i)});let x=document.querySelector(".tb-search-btn"),_=document.querySelector(".tb-search-input"),P=async()=>{let e=B;if(!e)return;let t=_.value.trim()||_.placeholder;if(!t||t.includes("...")){await showDialog({mode:"alert",text:"请输入搜索关键词。"});return}let a=await showDialog({mode:"confirm",text:`确定要搜索 "${t}" 吗？
这将刷新首页推荐流。`});a&&(_.value="",ai(e,t))};x&&x.addEventListener("click",P),_&&(_.removeAttribute("readonly"),_.addEventListener("keydown",e=>{"Enter"===e.key&&(e.preventDefault(),P())}));let H=document.getElementById("nav-to-profile");H&&H.addEventListener("click",()=>{ep("shoppingProfile"),al()});let R=document.getElementById("nav-to-profile-from-cart");R&&R.addEventListener("click",()=>{ep("shoppingProfile"),al()});let U=document.getElementById("nav-to-home-from-profile");U&&U.addEventListener("click",()=>{ep("shoppingHome")});let G=document.getElementById("nav-to-cart-from-profile");G&&G.addEventListener("click",()=>{let e=B;e&&(ep("shopping"),at(e))}),document.getElementById("cp-footprints-back-btn").addEventListener("click",()=>{ep("checkPhone")}),document.getElementById("cp-footprints-delete-btn").addEventListener("click",async()=>{let e=B;if(!e)return;let t=await showDialog({mode:"confirm",text:"确定要删除今日的足迹记录吗？\n删除后重新进入即可自动生成新的。"});t&&(w.logEntries=w.logEntries.filter(t=>!("footprints"===t.type&&t.author===e)),await w.persistLogToStorage(),await showDialog({mode:"alert",text:"记录已删除。"}),ep("checkPhone"))}),document.getElementById("cp-gallery-back-btn").addEventListener("click",()=>{ep("checkPhone")}),document.getElementById("cp-gallery-reset-btn").addEventListener("click",async()=>{let e=B;if(!e)return;let t=await showDialog({mode:"confirm",text:"确定要清空“近期回忆”吗？\n\n注意：\n1. 仅清空主相册，不会删除隐藏相册和回收站。\n2. 清空后重新进入可触发AI生成新内容。"});t&&(w.logEntries=w.logEntries.filter(t=>"gallery_update"!==t.type||t.author!==e&&(!t.content||t.content.author!==e)),await w.persistLogToStorage(),await showDialog({mode:"alert",text:"主相册已清空。"}),t4(e))});let W=document.getElementById("folder-hidden");W&&W.addEventListener("click",()=>{ep("hiddenAlbum"),Object.values(d).forEach(e=>e.classList.remove("active")),document.getElementById("cp-hidden-album-view").classList.add("active");let e=B;e&&tX(e)});let j=document.getElementById("folder-trash");j&&j.addEventListener("click",()=>{Object.values(d).forEach(e=>e.classList.remove("active")),document.getElementById("cp-trash-bin-view").classList.add("active");let e=B;e&&tK(e)}),document.getElementById("cp-hidden-back-btn").addEventListener("click",()=>{ep("gallery"),B&&t4(B)}),document.getElementById("cp-trash-back-btn").addEventListener("click",()=>{ep("gallery"),B&&t4(B)}),document.getElementById("cp-trash-clear-all").addEventListener("click",async()=>{let e=await showDialog({mode:"confirm",text:"确定要彻底清空所有废弃项目吗？"});if(e){let t=B;w.logEntries=w.logEntries.filter(e=>!("trash_bin_update"===e.type&&e.author===t)),await w.persistLogToStorage(),tK(t),await showDialog({mode:"alert",text:"垃圾桶已清空。"})}});let F=document.getElementById("cp-hidden-clear-btn");if(F&&F.addEventListener("click",async()=>{let e=B;if(!e)return;let t=await showDialog({mode:"confirm",text:"【警告】\n确定要清空隐藏相册的所有数据吗？此操作不可恢复。"});t&&(w.logEntries=w.logEntries.filter(t=>!("hidden_album_update"===t.type&&t.author===e)),await w.persistLogToStorage(),tX(e),await showDialog({mode:"alert",text:"隐藏空间已重置。"}))}),window._blmxListenersAdded)return;window._blmxListenersAdded=!0;let X=document.getElementById("global-image-viewer-img");X&&ex(X,e=>{e.stopPropagation(),X.src&&window.downloadImage(X.src)});let K,J=()=>clearTimeout(K);document.body.addEventListener("pointerup",J),document.body.addEventListener("pointerleave",J),document.body.addEventListener("pointercancel",J),document.body.addEventListener("click",e=>{e.target.matches(".nai-generated-image, .post-media-image, .image-url-bubble img")&&(e.stopPropagation(),n(e.target.src))},!0);let Z=document.querySelector(".widget-custom-text");Z&&Z.addEventListener("click",async()=>{let e=Z.textContent,t=await showDialog({mode:"prompt",text:"设置主屏幕自定义短语:",defaultValue:e});if(null!==t){let a=t.trim()||"风吹向海 我吹响自由";Z.textContent=a,localStorage.setItem(`blmx_hub_custom_text_${k}`,a),await showDialog({mode:"alert",text:"短语已更新！"})}}),document.getElementById("app-listen-together").addEventListener("click",()=>{ep("listenTogether")}),document.getElementById("lt-back-btn").addEventListener("click",()=>{ep("home")}),document.getElementById("lt-center-stage").addEventListener("click",function(){this.classList.toggle("show-lyrics")}),document.getElementById("lt-options-btn").addEventListener("click",()=>{tS()}),document.getElementById("lt-char-avatar").addEventListener("click",()=>{t0()});let et=document.querySelector(".lt-action-bar .fa-share-alt");et&&et.addEventListener("click",async()=>{let e={title:currentSong.title,artist:currentSong.artist,cover:A.avatar||"https://files.catbox.moe/bialj8.jpeg",src:currentSong.src,lrc:currentSong.lrc};ak(e,"shareMusic")});let ea=document.querySelector(".lt-action-bar .fa-download"),ei=document.getElementById("lt-playlist-overlay"),er=document.getElementById("lt-playlist-close-btn"),es=document.getElementById("lt-add-to-playlist-btn");ea&&ei&&(ea.addEventListener("click",()=>{tG(),ei.style.display="flex",setTimeout(()=>{ei.style.opacity="1"},10)}),er.addEventListener("click",()=>{ei.style.opacity="0",setTimeout(()=>{ei.style.display="none"},300)}),ei.addEventListener("click",e=>{e.target===ei&&(ei.style.opacity="0",setTimeout(()=>{ei.style.display="none"},300))}),es.addEventListener("click",async()=>{if(!currentSong||!currentSong.src){await showDialog({mode:"alert",text:"当前没有正在播放的歌曲。"});return}let e=await t1(currentSong);e&&(await showDialog({mode:"alert",text:`《${currentSong.title}》已加入歌单！`}),tG())})),document.getElementById("lt-mode-btn").addEventListener("click",tj),document.getElementById("lt-prev-btn").addEventListener("click",()=>{tY("prev",!1)}),document.getElementById("lt-next-btn").addEventListener("click",()=>{tY("next",!1)});let em=document.getElementById("lt-playlist-toggle-btn");em&&em.addEventListener("click",()=>{tG()});let eg=document.querySelector(".lt-action-bar .fa-heart");eg&&eg.addEventListener("click",function(){this.classList.contains("far")?(this.classList.replace("far","fas"),this.style.color="#E14438",this.style.transform="scale(1.2)",setTimeout(()=>{this.style.transform="scale(1)"},200)):(this.classList.replace("fas","far"),this.style.color="")});let eb=document.querySelector(".lt-action-bar .fa-comment-dots"),ev=document.getElementById("lt-input-overlay"),ew=document.getElementById("lt-chat-input"),eI=document.getElementById("lt-chat-send-btn"),e9=document.getElementById("lt-input-close-trigger");eb&&eb.addEventListener("click",()=>{ev.style.display="flex",ew.focus()}),e9.addEventListener("click",()=>{ev.style.display="none"}),eI.addEventListener("click",t_),ew.addEventListener("keydown",e=>{"Enter"===e.key&&(e.preventDefault(),t_())});let e$=document.getElementById("lt-play-pause-btn"),eL=document.getElementById("lt-vinyl-container"),eT=document.getElementById("lt-vinyl-view");e$.addEventListener("click",()=>{globalAudio.paused?(globalAudio.play(),e$.className="fas fa-pause-circle",eL.classList.add("playing"),eT.classList.add("playing")):(globalAudio.pause(),e$.className="fas fa-play-circle",eL.classList.remove("playing"),eT.classList.remove("playing"))});let eM=document.getElementById("lt-progress-slider");eM.addEventListener("mousedown",()=>isDraggingProgress=!0),eM.addEventListener("touchstart",()=>isDraggingProgress=!0),eM.addEventListener("input",()=>{let e=globalAudio.duration||0,t=eM.value/100*e;document.getElementById("lt-current-time").textContent=tP(t)});let eN=()=>{isDraggingProgress=!1;let e=globalAudio.duration||0;if(e>0){let t=eM.value/100*e;globalAudio.currentTime=t}};eM.addEventListener("change",eN),eM.addEventListener("touchend",eN);let eO=document.querySelector(".lt-controls .fa-list-ul"),e0=document.getElementById("lt-history-view");eO&&e0&&(eO.addEventListener("click",()=>{let e=e0.classList.toggle("active");e?(tD(),eO.style.color="var(--wechat-green-icon)"):eO.style.color=""}),e0.addEventListener("click",e=>{e.target===e0&&(e0.classList.remove("active"),eO.style.color="")}));let eH=[],eR=document.getElementById("cancel-call-btn"),eq=document.getElementById("end-call-btn"),eW=document.getElementById("in-call-send-btn"),ej=document.getElementById("in-call-input"),e7=document.getElementById("call-shared-screen"),e2=document.querySelector(".in-call-header");function eV(){let e=ej.value.trim(),t=eH.length>0;b?(eW.innerHTML='<i class="fas fa-spinner fa-spin"></i>',eW.disabled=!0,eW.style.backgroundColor="rgba(255,255,255,0.3)"):e.length>0?(eW.innerHTML='<i class="fas fa-paper-plane"></i>',eW.disabled=!1,eW.style.backgroundColor="rgba(255,255,255,0.3)"):t?(eW.innerHTML='<i class="fas fa-arrow-up"></i>',eW.disabled=!1,eW.style.backgroundColor="rgba(255,255,255,0.3)"):(eW.innerHTML='<i class="fas fa-paper-plane"></i>',eW.disabled=!0,eW.style.backgroundColor="rgba(255,255,255,0.3)")}async function e4(){if(0!==eH.length&&!b){b=!0,eX(),eV();try{let e=document.getElementById("chat-simulation-log"),t=Array.from(e.querySelectorAll(".chat-simulation-message")).slice(0,5).reverse().map(e=>{let t=e.classList.contains("me")?"{{user}}":q.name;return`${t}: ${e.textContent}`}).join("\n"),a=eH.map(e=>`{{user}}: ${e}`).join("\n");eH=[];let i=e7.querySelector(".screen-description")?.textContent||"画面中空无一物。",n=function e(t,a,i){let n="true"===localStorage.getItem(`blmx_nai_enabled_${k}`),l=n?`
    *   **[绘图指令增强 - 必须严格遵守]**:
...
(这将消耗一次API调用)`});if(R){let q=document.getElementById("profile-tab-ama").querySelector(".ama-qna-list");b=!0,H.textContent="思考中...";try{let U=function e(t){let a="user"===t||"{{user}}"===t?A:D.find(e=>e.id===t);if(!a)return console.error(`[AMA Context] 无法为 ${t} 创建上下文，角色不存在。`),null;let i=`
[任务: 匿名问答环节导演]
