...
5.  **[发送丰富消息]** (格式: \`角色ID:内容\`)
    ...
    *   \`角色ID:[图片:{"type":"desc","value":"对图片的中文详细描述"${n?',"ai_prompt":"用于绘图AI的英文Tag"':""}}]\`
    ${d}
    ${n?`
[绘图插件已启用]
当你要发送“图片”或“朋友圈”时，必须在 JSON 中额外增加 "ai_prompt" 字段。
"ai_prompt" 要求：直接书写英文 Tag，必须遵循 NovelAI (NAI Diffusion) 格式，描述构图、光影、氛围和动作细节。
示例: "ai_prompt": "sitting on bench, park background, cinematic lighting, masterpiece"
`:""}
    *   \`角色ID:[位置:具体的地点名称]\`
    *   \`角色ID:[文件:文件名.后缀]\`
...
8. **[朋友圈系统指令]**
    *   \`MOMENT:{"author":"角色ID","text":"朋友圈文字内容","timestamp":"YYYY-MM-DDTHH:mm","image_type":"desc","image":"图片描述"${n?',"ai_prompt":"用于绘图AI的英文Tag"':""},"isPrivate":布尔值,"visibleTo":["角色ID"],"invisibleTo":["角色ID"]}\`
    *   **评论指令 (CHAR_COMMENT)**:
...
`];n&&c.push(imageGenInstructions);let m=`
...
`}}}),c.push(m+"]\n"),e.forEach(t=>{c.push(`--- Conversation History for [${t}] ---`);let a=this.logEntries.filter(e=>{let a=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return a===t}).slice(-5).map(a=>this._formatEntryForAI(a,t,e)).filter(Boolean);c.push(...a),c.push("--- End of History ---\n")});else{let p=e[0],u=a.find(e=>e.id===p);if(!u)return"";let g=u.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");if("vgroup"===u.type){let y=u.members.filter(e=>"user"!==e);m+=` You are roleplaying in a virtual group chat named "${u.name}".
...
`;c.push(m);let f=this.logEntries.filter(e=>{let t=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return t===p}).slice(-5).map(t=>this._formatEntryForAI(t,p,e)).filter(Boolean);c.push(...f)}let h=this.logEntries.filter(e=>"MOMENT"===e.key&&e.data&&e.data.momentId&&!e.data.isPrivate).map(e=>`[MOMENT by ${e.data.author} (ID: ${e.data.momentId}) at ${e.data.timestamp}: "${e.data.text||"(no text)"}"]`).join("\n");h&&c.push("\n--- Recent Public Moments ---\n"+h);let b=`
[System Instruction: Your Turn]
**CRITICAL RULE:** This is a mobile phone simulation. Your response MUST ONLY be action commands or chat messages in the specified formats. ALL narrative descriptions (actions, thoughts, environment) are strictly forbidden. Do NOT repeat any previous messages from the history.`;return c.push(b),c.join("\n")}_formatEntryForAI(e,t,a){if("RECALL_MESSAGE"===e.key)return`[${e.data.author} recalled a message]`;if("forward"===e.type){let i=a.includes(t),n=`[${e.sender} forwarded content titled "${e.data.title}":
