...
`,i};r.forEach(e=>t.appendChild(m(e,"pinned"))),s.forEach(e=>t.appendChild(m(e,"hot"))),c.forEach(e=>t.appendChild(m(e,"")))}function e_(e){let t=document.getElementById("weibo-detail-view"),a=t.querySelector(".weibo-detail-body"),i=e.postId||e.target_post_id,n=O.posts.find(e=>e.postId===i);if(!n)return;let r=n.author,o=e.author.endsWith("BOT");if(o){let s=document.createElement("div");s.className="forum-bot-reply",s.innerHTML=`
            ...
            `,o.appendChild(t)}):f?o.innerHTML='<p style="text-align:center; color: var(--forum-text-secondary); padding: 2rem;">你还没有发布过任何帖子。</p>':o.innerHTML=`
<div style="text-align:center; color: var(--forum-text-secondary); padding: 2rem; display: flex; flex-direction: column; align-items: center; gap: 1rem;">
	<p style="margin: 0;">TA还没有发布过任何帖子。</p>
	<button id="force-profile-post-btn" class="studio-btn primary" style="font-size: 0.85em; padding: 0.5rem 1rem; border-radius: 20px;">
		<i class="fas fa-magic"></i> 催更动态
	</button>
</div>`;let f="user"===m.id||"{{user}}"===m.id;if(s.innerHTML="",f){let y=document.createElement("p");y.style.cssText="text-align:center; color: var(--forum-text-secondary); padding: 2rem;",y.textContent="这里是你的匿名提问箱。",s.appendChild(y)}else{let h=document.createElement("div");h.className="ama-input-box",h.innerHTML=`
        ...
        `,s.appendChild(h)}let b=document.createElement("div");if(b.className="ama-qna-list",s.appendChild(b),w){let v=w.logEntries.filter(t=>"AMA_PAIR"===t.key&&(t.data.author===e||c&&"{{user}}"===t.data.author)).sort((e,t)=>new Date(t.data.timestamp)-new Date(e.data.timestamp));if(f&&v.length>0){let E=s.querySelector("p");E&&(E.style.display="none")}v.forEach(e=>{let t=e.data,a=document.createElement("div");a.className="qna-card",a.innerHTML=`
			...
			`,q="",R=/#([^#\s]+)/g,G=[...(t.text||"").matchAll(R)].map(e=>e[1]);if(G.length>0){let U=[...new Set(G)];q=`
				...
				<div class="forum-content-panel" style="margin-top:0;">
					...
					`,z}{void 0===t.likes&&(t.likes=Math.floor(10*Math.random()));let Y,V,K=`<div class="forum-user-title" style="border-color: ${d.titleColor}; color: ${d.titleColor};"><i class="fas fa-certificate icon" style="color: ${d.titleColor};"></i><span>${d.title}</span></div>`;if(n)Y=`<div class="forum-user-panel" style="justify-content: flex-start; align-items: flex-start;"><img src="${d.avatar}" alt="${d.name}" class="forum-user-avatar" style="width: 2rem; height: 2rem; margin-bottom: 0;"><i class="fas fa-level-up-alt" style="transform: rotate(90deg); margin-top: 0.5rem; color: var(--forum-divider-color);"></i></div>`,V=`<div class="forum-content-panel"><div class="forum-post-body">${T}</div></div>`;else{let J;Y=`
						<div class="forum-user-panel">
							<img src="${d.avatar}" alt="${d.name}" class="forum-user-avatar">
...
`.trim()}(e);N=a;let i=await v({user_input:a,should_stream:!1});(M=i.trim())?await tO(M):t.innerHTML='<p style="text-align:center;color:#999;">胶卷曝光失败 (AI无返回)。</p>'}catch(n){console.error("Gallery generation failed:",n),t.innerHTML=`<p style="text-align:center;color:red;">错误: ${n.message}</p>`}finally{b=!1,e4()}}function t9(e){document.getElementById("cp-gallery-view");let t=document.getElementById("gallery-recent-grid"),a=document.getElementById("cp-gallery-title"),i=es(e,null);a.textContent=`${i}的相册`;let n=w.logEntries,r=[...n].reverse().find(t=>"gallery_update"===t.type&&t.author===e),o=[...n].reverse().find(t=>"hidden_album_update"===t.type&&t.author===e),s=[...n].reverse().find(t=>"trash_bin_update"===t.type&&t.author===e),d=o&&o.content.items?o.content.items.length:0,c=s&&s.content.items?s.content.items.filter(e=>!e.isRestored).length:0;if(document.querySelector("#folder-hidden .folder-count").textContent=d,document.querySelector("#folder-trash .folder-count").textContent=c,!r){t.innerHTML=`
...
</div>`,b=!0,e4();try{let a=function e(t){let a=es(t,null);return`
[任务: 生成加密相册内容]
角色: ${a} (ID: ${t})。
...
`.trim()}(e);N=a;let i=await v({user_input:a,should_stream:!1});(M=i.trim())?await tO(M):t.innerHTML='<p style="color:#999; text-align:center;">垃圾桶是空的。</p>'}catch(n){console.error("Trash Bin generation failed:",n),t.innerHTML=`<p style="color:red; text-align:center;">错误: ${n.message}</p>`}finally{b=!1,e4()}}function tk(e){let t=document.getElementById("trash-list-container");t.innerHTML="";let a=[...w.logEntries].reverse().find(t=>"trash_bin_update"===t.type&&t.author===e);if(!a){t.innerHTML=`
...
`,b=!0,e4();try{let a=function e(t){let a=es(t,null);return`
[任务: 生成手机购物车内容]
角色: ${a} (ID: ${t})。
...
`,b=!0,e4();try{let l=function e(t,a=null,i=null){let n=es(t,null),l="",r="";return a?l=`用户刚刚搜索了关键词【${a}】。请生成 4 个与该关键词高度相关但风格各异的搜索结果商品。同时将 "${a}" 插入到 history 搜索历史数组的第一位。`:i?(l=`用户点击了首页的【${i.name}】频道。请根据该频道的特性，为角色推荐 4 个商品。`,i.instruction&&(r=`
**【频道特殊要求】**:
