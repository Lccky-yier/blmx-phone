...
`}));let c=[`[ABSOLUTE HIGHEST PRIORITY RULE]
...
2.  **[角色通用行为]**
    * 更新个性签名：\`SIGNATURE_UPDATE:{"author":"角色ID","signature":"新的个性签名"}\`
    * 主动撤回消息：\`RECALL_MESSAGE:{"author":"角色ID","target_text":"要撤回的完整消息文本"}\`

3.  **[发起视频通话]**
    *   **用途**: 当一个角色想主动给{{user}}打视频电话时使用。
    *   **格式**: \`VIDEO_CALL:{"caller":"发起方角色ID","receiver":"接收方ID"}\`

4.  **[发送丰富消息]** (格式: \`角色ID:内容\`)
    * 发送语音：\`角色ID:[语音:{"text":"语音转写的文字","duration":整数秒数}]\`
    * 发送图片：\`角色ID:[图片:{"type":"desc","value":"对图片的详细描述"${n?', "ai_prompt":"English tags"':""}}]\`
    * 发送位置：\`角色ID:[位置:具体的地点名称]\`
    * 发送文件：\`角色ID:[文件:文件名.后缀]\`
    * 发送文件：\`角色ID:[sticker:表情名称]\`(注意：表情名称必须严格来自表情包列表，禁止虚构)
    * 转发消息或朋友圈：\`角色ID:[forward:{"title":"转发的标题","messageIds":["消息ID_1", "朋友圈的唯一momentId"]}]\`

5.  **[转账与礼物]** (格式: \`角色ID:内容\`)
    * 发起转账：\`角色ID:[转账:{"amount":金额,"note":"备注","recipientId":"接收方ID(群聊必须)","status":"sent"}]\`
    * 赠送礼物：\`角色ID:[礼物:{"name":"礼物名","price":"价格(可选)","recipientId":"接收方ID(群聊必须)","status":"sent"}]\`
    * **回应\`{{user}}\` (必须回应):**
      * 接收转账：\`角色ID:[转账:{"amount":"金额","status":"accepted"}]\`
      * 收下礼物：\`角色ID:[礼物:{"name":"礼物名称","status":"accepted"}]\`
      * 拒收/退还：将\`status\` 改为  \`rejected\`

6. **[群聊管理指令]**
    * 创建群聊：\`CREATE_GROUP:{"name":"群聊名称","owner":"创建者角色ID","members":["角色ID_1"],"include_user":布尔值}\`
    * 邀请成员：\`INVITE_MEMBER:{"author":"邀请人ID","convoId":"群聊ID","targetId":"被邀请人ID"}\`
    * 踢出成员：\`KICK_MEMBER:{"author":"操作者ID","convoId":"群聊ID","targetId":"被踢者ID"}\`
    * 退出群聊：\`LEAVE_GROUP:{"author":"要退群的角色ID","convoId":"群聊ID"}\`
    * 设置管理员：\`SET_ADMIN:{"author":"操作者ID","convoId":"群聊ID","targetId":"被设为管理的ID"}\`
    * 禁言成员：\`MUTE_MEMBER:{"author":"操作者ID","convoId":"群聊ID","targetId":"被禁言ID","duration":禁言分钟数}\`
    * 解除禁言：\`UNMUTE_MEMBER:{"author":"操作者ID","convoId":"群聊ID","targetId":"被解除禁言者ID"}\`
    * 修改群昵称：\`CHANGE_NICKNAME:{"author":"操作者ID","convoId":"群聊ID","targetId":"目标ID","newName":"新群昵称"}\`
    * 修改群聊名：\`RENAME_GROUP:{"author":"操作者ID","convoId":"群聊ID","newName":"新群聊名称"}\`

7. **[朋友圈系统指令]**
    * 发朋友圈：\`MOMENT:{"author":"角色ID","text":"朋友圈文字内容","timestamp":"YYYY-MM-DDTHH:mm","image_type":"desc","image":"图片描述(可选)"${n?', "ai_prompt":"English tags"':""},"isPrivate":布尔值,"visibleTo":["角色ID"],"invisibleTo":["角色ID"]}\`
    * 评论朋友圈：\`CHAR_COMMENT:{"author":"评论者ID","text":"[引用:\\"发布者ID: 朋友圈原文\\"] 你的评论内容"}\`
    * **评论规则 (CRITICAL)**:使用引用格式来指定你要评论哪条朋友圈。

8.  **[红包与事件]**
    *   红包被发出后，系统会自动通过\`GROUP_EVENT\`消息模拟领取过程。你应将这些事件视为红包已被处理，**严禁**再使用\`EVENT_LOG\`或其他方式重复模拟领取动作。
    ${l}
`],m=`
[System Note: You are roleplaying inside a chat app. The user's ID is '{{user}}'.`;if(i)m+=` You MUST account for ALL conversations listed below that have new messages from '{{user}}'. CRITICAL: ALL direct replies MUST be prefixed with the conversation ID, e.g., '[convo_id] speaker_id: message'.

Active Conversations:
`,e.forEach(e=>{let i=a.find(t=>t.id===e);if(i){let n="Private Chat";if("group"===i.type?n=`Group Chat "${i.name}"`:"vgroup"===i.type&&(n=`Virtual Group Chat "${i.name}"`),"vgroup"===i.type)m+=`- ID: [${i.id}], Type: ${n}, Background Members: "${i.background_members_desc}"
`;else{let l=i.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");m+=`- ID: [${i.id}], Type: ${n}, Participants: ${l}
`}}}),c.push(m+"]\n"),e.forEach(t=>{let a=this.logEntries.filter(e=>{let a=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return a===t}),i=[];for(let n=a.length-1;n>=0;n--){let l=a[n];if("user"===l.sender||"{{user}}"===l.sender)i.unshift(l);else break}c.push(`
--- [会话 ${t}] 的指令 ---`),i.length>0?(c.push(`以下是 {{user}} 的最新回复：`),i.forEach(a=>{let i=this._formatEntryForAI(a,t,e);i&&c.push(i)}),c.push(`请根据上述内容，给出你的回应。`)):c.push(`当前轮到你发言，请主动推进对话。`)});else{let p=e[0],u=a.find(e=>e.id===p);if(!u)return"";let g=u.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");if("vgroup"===u.type){let f=u.members.filter(e=>"user"!==e);m+=` You are roleplaying in a virtual group chat named "${u.name}".
*   **Core Members Present**: ${f.length>0?f.join(", "):"None"}.
*   **Background Members**: You can freely roleplay any number of background members fitting this description: "${u.background_members_desc}".
...
`}else m+=` You are roleplaying in a single chat. Participants: ${g}. Respond as one of the characters (excluding '{{user}}'). Prefix each line with the speaker's ID.]

`;c.push(m);let y=this.logEntries.filter(e=>{let t=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return t===p}),h=[];for(let b=y.length-1;b>=0;b--){let v=y[b];if("user"===v.sender||"{{user}}"===v.sender)h.unshift(v);else break}h.length>0?(c.push(`
[以下是 {{user}} 的最新回复：]`),h.forEach(t=>{let a=this._formatEntryForAI(t,p,e);a&&c.push(a)}),c.push(`
[请根据上述内容，继续给出你的自然回应]`)):c.push(`
[请根据当前聊天氛围，主动推进对话]`)}let w=this.logEntries.filter(e=>"MOMENT"===e.key&&e.data&&e.data.momentId&&!e.data.isPrivate).map(e=>`[MOMENT by ${e.data.author} (ID: ${e.data.momentId}) at ${e.data.timestamp}: "${e.data.text||"(no text)"}"]`).join("\n");w&&c.push("\n--- Recent Public Moments ---\n"+w);let E=`
[System Instruction: Your Turn]
**CRITICAL RULE:** This is a mobile phone simulation. Your response MUST ONLY be action commands or chat messages in the specified formats. ALL narrative descriptions (actions, thoughts, environment) are strictly forbidden. Do NOT repeat any previous messages from the history.`;return c.push(E),c.join("\n")}_formatEntryForAI(e,t,a){if("RECALL_MESSAGE"===e.key)return`[${e.data.author} recalled a message]`;if("forward"===e.type){let i=a.includes(t),n=`[${e.sender} forwarded content titled "${e.data.title}":
...
<div class="red-packet-card">
	...
	<div class="red-packet-footer">微信红包</div>
					...
					`,k+=" voice-bubble";break;default:let eS=t.content.match(/^\[引用:\s*"(.*?):\s*(.*?)"\]\s*(.*)$/s);if(eS){let eD=eS[1],eC=eS[2],eM=eS[3],eA=eC.match(/^\[语音:\s*(.*)\]$/);if(eA)try{let eN=eA[1].replace(/\\"/g,'"'),e0=JSON.parse(eN);eC=`语音：${e0.text}`}catch(eP){console.error("解析引用的语音消息失败:",eP,"原始字符串:",eA[1]),eC="[语音]"}else eC.startsWith("[表情:")?eC="[表情]":eC.startsWith("[图片:")?eC="[图片]":eC.startsWith("[位置:")&&(eC="[位置]");L=`
								<div class="quote-container">
									<div class="quote-author">${eD}</div>
									<div class="quote-content">${eC.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div>
								</div>
								<span class="reply-text">${eM.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</span>
...
`,i};r.forEach(e=>t.appendChild(m(e,"pinned"))),s.forEach(e=>t.appendChild(m(e,"hot"))),c.forEach(e=>t.appendChild(m(e,"")))}function e_(e){let t=document.getElementById("weibo-detail-view"),a=t.querySelector(".weibo-detail-body"),i=e.postId||e.target_post_id,n=O.posts.find(e=>e.postId===i);if(!n)return;let r=n.author,o=e.author.endsWith("BOT");if(o){let s=document.createElement("div");s.className="forum-bot-reply",s.innerHTML=`
        ...
        `,s.appendChild(h)}let b=document.createElement("div");if(b.className="ama-qna-list",s.appendChild(b),w){let v=w.logEntries.filter(t=>"AMA_PAIR"===t.key&&(t.data.author===e||c&&"{{user}}"===t.data.author)).sort((e,t)=>new Date(t.data.timestamp)-new Date(e.data.timestamp));if(f&&v.length>0){let E=s.querySelector("p");E&&(E.style.display="none")}v.forEach(e=>{let t=e.data,a=document.createElement("div");a.className="qna-card",a.innerHTML=`
			...
			`,q="",R=/#([^#\s]+)/g,G=[...(t.text||"").matchAll(R)].map(e=>e[1]);if(G.length>0){let U=[...new Set(G)];q=`
				...
				</div>`;B.innerHTML=A+H+j}else{let z=t.author.endsWith("BOT");if(z){let F=document.createElement("div");return F.className="forum-bot-reply",F.innerHTML=`
						<i class="fas fa-robot bot-icon"></i>
						<div class="bot-content-wrapper">
							<div class="bot-author-name">${t.author}</div>
							<div class="bot-text">${t.text||t.content}</div>
						</div>
					`,F}{void 0===t.likes&&(t.likes=Math.floor(10*Math.random()));let Y,V,K=`<div class="forum-user-title" style="border-color: ${d.titleColor}; color: ${d.titleColor};"><i class="fas fa-certificate icon" style="color: ${d.titleColor};"></i><span>${d.title}</span></div>`;if(n)Y=`<div class="forum-user-panel" style="justify-content: flex-start; align-items: flex-start;"><img src="${d.avatar}" alt="${d.name}" class="forum-user-avatar" style="width: 2rem; height: 2rem; margin-bottom: 0;"><i class="fas fa-level-up-alt" style="transform: rotate(90deg); margin-top: 0.5rem; color: var(--forum-divider-color);"></i></div>`,V=`<div class="forum-content-panel"><div class="forum-post-body">${T}</div></div>`;else{let J;Y=`
						<div class="forum-user-panel">
							<img src="${d.avatar}" alt="${d.name}" class="forum-user-avatar">
...
`}});let c=t+a+n+r+s;return c}();N=e;let t=await v({user_input:e,should_stream:!1});A=t;let a="AI未能生成有效的总结。历史记录未被清除。",i=!1;try{let n=JSON.parse(t.trim());n.summary&&"string"==typeof n.summary&&n.summary.trim()&&(a=`【历史存档摘要】:
...
${t.trim()}`}if(i){let r=w.messageId;await window.parent.TavernHelper.setChatMessage(a,r,{refresh:"none"}),w.logEntries=[],w.messageId=await window.parent.TavernHelper.getLastMessageId(),eN(),eQ(),tt(),d.wechatChat.classList.contains("active")&&eI(T),await showDialog({mode:"alert",text:"历史记录已成功总结并存档！手机性能已恢复。"})}else await showDialog({mode:"alert",text:a})}catch(o){console.error("[Archive] AI summary generation failed:",o),await showDialog({mode:"alert",text:`AI总结失败: ${o.message}`})}finally{b=!1,e7()}}function eH(){return new Promise(e=>{let t=document.getElementById("group-chat-modal"),a=document.getElementById("group-chat-contact-list-container"),i=document.getElementById("group-chat-modal-header"),n=document.getElementById("group-chat-modal-footer");a.innerHTML="",i.querySelector(".title").textContent="选择一个作者",i.querySelector("#group-chat-confirm-btn").style.display="none",n.style.display="none";let l=i=>{let n=a.cloneNode(!1);a.parentNode.replaceChild(n,a),t.style.display="none",e(i)},r=i.querySelector("#group-chat-cancel-btn"),o=r.cloneNode(!0);r.parentNode.replaceChild(o,r),o.onclick=()=>l(null),D.forEach(e=>{let t=document.createElement("div");t.className="group-chat-contact-item",t.style.cursor="pointer",t.innerHTML=`
                <img src="${ed(e.id)}" alt="${es(e.id,null)}">
                <span>${es(e.id,null)}</span>
            `,t.addEventListener("click",()=>{l(e.id)}),a.appendChild(t)}),t.style.display="flex"})}function e3(e){let t=tR(),a=t.find(t=>t.id===e);return a&&a.communityBible?a.communityBible:(console.warn(`[AI Context] Could not find community bible for category: ${e}. Using fallback.`),"【错误】未找到对应的社区说明。请基于通用知识进行创作。")}async function eq(){if(b){await showDialog({mode:"alert",text:"AI正在生成中，请稍后再试。"});return}document.getElementById("weibo-feed-view");let e=document.getElementById("weibo-feed-title"),t=e.textContent,a="",n=Array.from(document.querySelectorAll(".weibo-zone-card")).find(e=>e.querySelector(".zone-title").textContent===t);if(n)a=n.dataset.category;else{await showDialog({mode:"alert",text:"错误：无法确定当前帖子分区。"});return}let l=await tU({title:"发布新微博",fields:[{id:"title",label:"帖子标题",type:"text"},{id:"body",label:"帖子正文",type:"textarea"},{id:"image_url",label:"图片链接 (可选, 优先使用)",type:"text"},{id:"image_desc",label:"或 图片描述 (可选)",type:"text"}]});if(null===l)return;let{title:r,body:o,image_url:s,image_desc:d}=l,c="",m="none";if(s&&s.trim()?(m="url",c=s.trim()):d&&d.trim()&&(m="desc",c=d.trim()),!r.trim()&&!o.trim()&&!c.trim()){await showDialog({mode:"alert",text:"标题、正文和图片至少需要填写一项。"});return}let p=await showDialog({mode:"confirm",text:"如何发布这篇帖子？\n\n(确定 = 匿名发布)\n(取消 = 实名发布)"}),u;if(p){let g=tw();if(g&&g.name)u=g.name;else{await showDialog({mode:"alert",text:"请先通过右上角的齿轮设置您的马甲才能匿名发帖。"});return}}else u=M.id;let f={author:u,title:r,text:o,image_type:m,image:c,postId:`weibo_${Date.now()}_${Math.random().toString(36).substr(2,9)}_${M.id}`,category:a,timestamp:new Date(window.currentGameDate).toISOString(),likes:Math.floor(100*Math.random()),retweets:Math.floor(21*Math.random()),bookmarks:Math.floor(11*Math.random())};w.addEntry({key:"WEIBO_POST",data:f}),"desc"===f.image_type&&f.image&&i(f.postId,f.image,"weibo"),await w.persistLogToStorage(),eN(),eT(a),em("weiboFeed",{category:a,categoryName:t}),await showDialog({mode:"alert",text:`新帖子已成功发布！`})}async function eR(){if(b){await showDialog({mode:"alert",text:"AI正在思考中，请稍后再试。"});return}let e=await showDialog({mode:"confirm",text:"是否要让时间流逝，看看角色们发生了什么？\n(这将消耗一次API调用来生成全局动态)"});if(e){b=!0,e7(),await showDialog({mode:"alert",text:"正在捕捉世界动态，请稍候..."});try{let t=function e(){let t=tR(),a=t.map(e=>`- 【${e.title}】(ID: ${e.id})`).join("\n"),i=`
[任务：世界动态导演]
...
`,e.querySelector(".sp-delete-btn").onclick=async()=>{let t=e.querySelectorAll(".item-checkbox:checked");if(0===t.length)return;let a=await showDialog({mode:"confirm",text:`确定要删除这 ${t.length} 个商品吗？`});if(!a)return;let i=S,n=[...w.logEntries].reverse().find(e=>"shopping_update"===e.type&&e.author===i);if(!n)return;let l=[];t.forEach(e=>{let t=e.closest(".sp-item");l.push(parseInt(t.dataset.itemIndex,10))});let r=n.content.items,o=r.filter((e,t)=>!l.includes(t));w.logEntries=w.logEntries.filter(e=>!("shopping_update"===e.type&&e.author===i));let s=`msg-shopping-del-${Date.now()}`;w.addEntry({id:s,type:"shopping_update",author:i,content:{author:i,items:o},timestamp:new Date(window.currentGameDate).toISOString()}),await w.persistLogToStorage(),tB(),tD(i),await showDialog({mode:"alert",text:"删除成功。"})}}function tD(e){let t=document.getElementById("cp-shopping-view"),a=t.querySelector(".shopping-body"),i=t.querySelector(".sp-total-price span"),r=t.querySelector(".sp-checkout-btn"),o=t.querySelector(".sp-footer-left .sp-checkbox"),s=[...w.logEntries].reverse().find(t=>"shopping_update"===t.type&&t.author===e),d=s&&s.content&&s.content.items?s.content.items:[];a.innerHTML="";let c=es(e,null);if(t.querySelector(".settings-header .title").textContent=`${c}的购物车 (${d.length})`,0===d.length){a.innerHTML=`
        ...
        `;t.forEach(e=>{let t=e._index,a=s&&s.id?`${s.id}_img_${t}`:`temp_img_${t}`,i=l(a,e.image),n=!1;if(i)i.includes("<img");else if(e.image&&(e.image.startsWith("http")||e.image.startsWith("blob:")))i=`<img src="${e.image}" style="width:100%; height:100%; object-fit:cover; display:block;" onerror="this.onerror=null;this.src='https://i.postimg.cc/Cx6hDcwn/c41va3.jpg';">`;else{n=!0;let o=(e.image||"").replace(/"/g,"&quot;");i=`<div class="sp-text-placeholder" data-full-text="${o}" style="background: url('https://i.postimg.cc/Cx6hDcwn/c41va3.jpg') center/cover no-repeat; color: transparent;">${e.image}</div>`}r+=`
...
合计：\xa5${r.toFixed(2)}`});if(!o)return;let s=w.logEntries.find(e=>e.id===t);s&&s.content&&s.content.items&&(l.sort((e,t)=>t-e),l.forEach(e=>{s.content.items.splice(e,1)}),await w.persistLogToStorage());let d={payer:M.name,receiver:e,items:n,total:r.toFixed(2),timestamp:new Date().toLocaleString()},c=C.find(t=>"single"===t.type&&t.members.includes(e));c||(c={id:`convo_single_${e}`,type:"single",members:["user",e]},C.push(c)),w.addEntry({type:"payment_receipt",sender:"user",conversationId:c.id,content:JSON.stringify(d),timestamp:new Date(window.currentGameDate).toISOString()}),await w.persistLogToStorage(),en(),await showDialog({mode:"alert",text:"支付成功！回执单已发送给对方。"}),tD(e),em("wechatChat",{conversationId:c.id})}async function tM(e,t=null,a=null){if(b)return;let i=document.getElementById("shopping-home-feed"),n="正在刷新推荐流...";t?n=`正在搜索 "${t}"...`:a&&(n=`正在进入【${a.name}】频道...`),i.innerHTML=`
...
`.trim()}(e,t,a);N=l;let r=await v({user_input:l,should_stream:!1});(A=r.trim())?await tO(A):i.innerHTML='<p style="text-align:center;color:#999; grid-column:1/-1;">加载失败 (AI无返回)。</p>'}catch(o){console.error("Shopping Home generation failed:",o),i.innerHTML=`<p style="text-align:center;color:red; grid-column:1/-1;">错误: ${o.message}</p>`}finally{b=!1,e7()}}function tA(e){let t=document.getElementById("shopping-home-feed");if(!t)return;let a=[...w.logEntries].reverse().find(t=>"taobao_home"===t.type&&t.author===e);if(!a){t.innerHTML=`
...
`;let i=document.getElementById("init-tb-home-btn");i&&(i.onclick=()=>tM(e));return}let n=a.content;t.innerHTML="",n.items&&n.items.length>0&&n.items.forEach((e,i)=>{let n=`${a.id}_img_${i}`,r=l(n,e.image);if(!r){let o=(e.image||"").replace(/"/g,"&quot;");r=`<div class="tb-img-placeholder-text" data-full-text="${o}" style="background: url('https://i.postimg.cc/Cx6hDcwn/c41va3.jpg') center/cover no-repeat;"></div>`}let s=document.createElement("div");s.className="tb-product-card",s.innerHTML=`
...
[你的指令]
现在，请严格遵循以上格式和示例，为【${a}】分区生成初始的帖子列表。`;return n.trim()}(e);if(!t){b=!1,e7();return}N=t;try{let a=await v({user_input:t,should_stream:!1});(A=a.trim())&&await tO(A,{contextCategory:e})}catch(i){console.error("[BLMX Forum] AI initial feed generation failed:",i),await showDialog({mode:"alert",text:`AI生成初始帖子列表失败: ${i.message}`})}finally{b=!1,e7()}}async function tH(e){if(b){console.warn("[BLMX Forum] AI is already generating. Aborting full post generation.");return}let t=O.posts.find(t=>t.postId===e);if(!t)return;console.log(`[BLMX Forum] Triggering full post generation for postId: ${e}`),b=!0,e7();let a=function e(t){let a=O.posts.find(e=>e.postId===t);if(!a)return console.error(`[BLMX Forum] 无法为帖子 ${t} 创建上下文，帖子未找到。`),null;let i="true"===localStorage.getItem(`blmx_nai_enabled_${k}`),n=document.querySelector(`.weibo-zone-card[data-category="${a.category}"] .zone-title`).textContent,l=e3(a.category),r=i?`
* **[绘图指令增强 - 必须严格遵守]**:
...
[你的指令]
现在，请沉浸到这个论坛里，严格遵循以上所有规则，开始你的创作。`;return o.trim()}(e);if(!a){b=!1,e7();return}N=a,await showDialog({mode:"alert",text:"AI正在撰写帖子内容和评论，请稍候..."});try{let i=await v({user_input:a,should_stream:!1});(A=i.trim())&&await tO(A,{contextCategory:t.category,contextPostId:e})}catch(n){console.error("[BLMX Forum] AI full post generation failed:",n),await showDialog({mode:"alert",text:`AI生成帖子内容失败: ${n.message}`})}finally{b=!1,e7()}}async function t3(e){if(b||!w||!v){console.warn("[BLMX Weibo] AI is already generating or essential functions are missing. Aborting.");return}console.log(`[BLMX Weibo] Triggering AI response for post: ${e}`),b=!0,e7();let t=function e(t){let a=O.posts.find(e=>e.postId===t);if(!a)return null;let i=`
* **【最高优先级：核心禁令】**:
1. **人设严格遵守 (OOC禁令)**: 在扮演任何核心角色 (char) 发表评论前，你 **必须** 深入思考该角色的性格和背景。 **绝对禁止** 做出任何不符合角色人设的行为（OOC）。
2. **用户身份禁令**: 用户的ID是 '{{user}}'。你 **绝对禁止** 以 '{{user}}' 的身份或名义生成任何评论、点赞或任何形式的互动。`,n=e3(a.category),l=`
...
`;let c=/\[引用:"(?:.|\n)*?"\]\s*/;d.forEach(e=>{let t=e.author===M.id||"user"===e.author||"{{user}}"===e.author?"{{user}}":es(e.author,null),a=(e.text||"").replace(c,"");r+=`* [作者: ${t}]: ${a}
`})}else r+=`* **最新评论**: 当前暂无评论。
`;let m=`
...
* **【第一法则：拒绝闭环，无限发散】**:
**严禁车轱辘话**：禁止对他人的观点进行单纯的赞同、复读或附和。每一条新评论都必须提供**信息增量。可以继续主楼话题，也可以歪楼引出新话题。**

* **【第二法则：高流量广场效应】**:
**打破朋友圈感**：默认这是一个高流量的公共版块。要营造出“众声喧哗”的真实热闹感，要有五花八门的网友和NPC，要有互动感。

* **【第三法则：解锁全网知识库】**:
**注入网感**：不要局限于当前文本。请充分调用训练数据（互联网文化、流行梗、生活常识），让发言带有“网感”和不可预测性。允许神展开，允许脑洞大开。

* **【内容与产出配额】**:
* **评论总数**: 生成“总计 **3-5** 条”评论。
* **字数限制**: **保持短小精悍**。每条评论的理想长度在 **0到50个汉字之间**。绝对禁止长篇大论，模拟真实的碎片化回复。`,p=`
...
(这将消耗一次API调用)`});if(q){let R=document.getElementById("profile-tab-ama").querySelector(".ama-qna-list");b=!0,H.textContent="思考中...";try{let G=function e(t){let a="user"===t||"{{user}}"===t?M:D.find(e=>e.id===t);if(!a)return console.error(`[AMA Context] 无法为 ${t} 创建上下文，角色不存在。`),null;let i=`
[任务: 匿名问答环节导演]
