...
9. **[朋友圈系统指令]**
    ...
    *   **引用规则 (CRITICAL)**: 必须使用引用格式来指定你要评论哪条动态。若目标动态没有文字（纯图），请引用其图片描述。这里的 \`[引用:...]\` **必须且只能** 是该条朋友圈的**正文内容**，用于表明你在评论哪条动态。
    *   **绝对禁止** 引用其他人的**评论**（禁止套娃）。
    *   **回复他人**: 如果是回复评论区里的其他人，请在引用**原文**之后，使用 \`@对方名字\`。正确示例: \`[引用:"楼主: 今天天气真好"] @路人甲 是啊，适合野餐。\`

10.  **[红包与事件]**
    *   红包被发出后，系统会自动通过\`GROUP_EVENT\`消息模拟领取过程。你应将这些事件视为红包已被处理，**严禁**再使用\`EVENT_LOG\`或其他方式重复模拟领取动作。
`],d=`
[System Note: You are roleplaying inside a chat app. The user's ID is '{{user}}'.`;if(i)d+=` You MUST account for ALL conversations listed below that have new messages from '{{user}}'. CRITICAL: ALL direct replies MUST be prefixed with the conversation ID, e.g., '[convo_id] speaker_id: message'.

Active Conversations:
`,e.forEach(e=>{let i=a.find(t=>t.id===e);if(i){let n="Private Chat";if("group"===i.type?n=`Group Chat "${i.name}"`:"vgroup"===i.type&&(n=`Virtual Group Chat "${i.name}"`),"vgroup"===i.type)d+=`- ID: [${i.id}], Type: ${n}, Background Members: "${i.background_members_desc}"
`;else{let l=i.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");d+=`- ID: [${i.id}], Type: ${n}, Participants: ${l}
`}}}),s.push(d+"]\n"),e.forEach(t=>{s.push(`--- Conversation History for [${t}] ---`);let a=this.logEntries.filter(e=>{let a=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return a===t}).slice(-5).map(a=>this._formatEntryForAI(a,t,e)).filter(Boolean);s.push(...a),s.push("--- End of History ---\n")});else{let c=e[0],m=a.find(e=>e.id===c);if(!m)return"";let p=m.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");if("vgroup"===m.type){let u=m.members.filter(e=>"user"!==e);d+=` You are roleplaying in a virtual group chat named "${m.name}".
*   **Core Members Present**: ${u.length>0?u.join(", "):"None"}.
*   **Background Members**: You can freely roleplay any number of background members fitting this description: "${m.background_members_desc}".
...
`}else d+=` You are roleplaying in a single chat. Participants: ${p}. Respond as one of the characters (excluding '{{user}}'). Prefix each line with the speaker's ID.]

`;s.push(d);let g=this.logEntries.filter(e=>{let t=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return t===c}).slice(-5).map(t=>this._formatEntryForAI(t,c,e)).filter(Boolean);s.push(...g)}let y=this.logEntries.filter(e=>"MOMENT"===e.key&&e.data&&e.data.momentId&&!e.data.isPrivate).map(e=>`[MOMENT by ${e.data.author} (ID: ${e.data.momentId}) at ${e.data.timestamp}: "${e.data.text||"(no text)"}"]`).join("\n");y&&s.push("\n--- Recent Public Moments ---\n"+y);let f=`
[System Instruction: Your Turn]
**CRITICAL RULE:** This is a mobile phone simulation. Your response MUST ONLY be action commands or chat messages in the specified formats. ALL narrative descriptions (actions, thoughts, environment) are strictly forbidden. Do NOT repeat any previous messages from the history.`;return s.push(f),s.join("\n")}_formatEntryForAI(e,t,a){if("RECALL_MESSAGE"===e.key)return`[${e.data.author} recalled a message]`;if("forward"===e.type){let i=a.includes(t),n=`[${e.sender} forwarded content titled "${e.data.title}":
...
现在，请开始你的总结任务。
`;return i.trim()}(q.id,a);O=i;let n=await v({user_input:i,should_stream:!1});M=n.trim();let l=JSON.parse(n.trim());l.summary&&(t=l.summary)}catch(r){console.error("[Call Summary] AI summary generation failed:",r),t="（AI未能成功总结通话内容）"}finally{b=!1,eX()}}(function e(t=""){if(!q.id)return;let a=C.find(e=>"single"===e.type&&e.members.includes(q.id));if(!a)return;let i=Math.floor(R/60).toString().padStart(2,"0"),n=(R%60).toString().padStart(2,"0"),l=`${i}:${n}`,r=`视频通话已结束，通话时长 ${l}`,o={convoId:a.id,timestamp:new Date(window.currentGameDate).toISOString().slice(0,16).replace("T"," "),description:r,...t&&{callSummary:t}};w.addEntry({type:"event_log",content:o}),w.persistLogToStorage(),T===a.id&&d.wechatChat.classList.contains("active")&&e8(o,w.logEntries.length-1),eS(a.id,o.timestamp)})(t),ev(),eg(null),P="idle",q={id:null,name:null,avatar:null}}async function ef(){console.log("[Call] User accepted the call. Connecting..."),P="in-call";let e=[...w.logEntries].reverse().find(e=>"VIDEO_CALL"===e.key&&e.data.caller===q.id);e&&(e.data.status="answered",w.persistLogToStorage());let t=document.getElementById("in-call-screen");t.querySelector(".caller-name").textContent=q.name,j=[],z=-1,F("image",q.avatar),eg("in-call-screen"),eb(),b=!0,eX(),updateInCallButtonUI();try{let a=function e(t){let a=`
...
`;return a.trim()}(q.id);O=a;let i=await v({user_input:a,should_stream:!1});M=i.trim();let n=document.getElementById("chat-simulation-log");n.innerHTML="";let l=i.trim().split("\n");for(let r of l){if(!r)continue;let o=/UPDATE_CALL_SCREEN:({.*})/,s=r.match(o);if(s)eu(JSON.parse(s[1]));else{let d=document.createElement("div");d.className="chat-simulation-message",d.textContent=r,n.insertBefore(d,n.firstChild)}}}catch(c){console.error("[Call Accept] AI failed to respond after call accept:",c)}finally{b=!1,eX(),updateInCallButtonUI()}}function eh(){console.log("[Call] User declined the call.");let e=[...w.logEntries].reverse().find(e=>"VIDEO_CALL"===e.key&&e.data.caller===q.id);e&&(e.data.status="declined",w.persistLogToStorage());let t=C.find(e=>"single"===e.type&&e.members.includes(q.id));if(t){let a={convoId:t.id,timestamp:new Date(window.currentGameDate).toISOString().slice(0,16).replace("T"," "),description:`你拒接了 ${q.name} 的视频通话邀请。`};w.addEntry({type:"event_log",content:a}),w.persistLogToStorage(),eS(t.id,a.timestamp)}P="idle",eg(null)}function eb(){ev();let e=document.getElementById("call-timer");e&&(R=0,e.textContent="00:00",H=setInterval(()=>{R++;let t=Math.floor(R/60).toString().padStart(2,"0"),a=(R%60).toString().padStart(2,"0");e.textContent=`${t}:${a}`},1e3))}function ev(){clearInterval(H),H=null}function ew(e){let t=_.find(t=>t.id===e);if(!t)return;document.getElementById("contact-details-profile-card").dataset.contactId=t.id,document.getElementById("contact-details-avatar").src=ec(t.id),document.getElementById("contact-details-name").textContent=ed(t.id,null),document.getElementById("contact-details-avatar").dataset.contactId=t.id;let a=document.getElementById("set-private-chat-wallpaper-btn"),i=C.find(e=>"single"===e.type&&e.members.includes(t.id));i?(a.style.display="flex",a.dataset.convoId=i.id):a.style.display="none";let n=document.querySelector("#block-contact-btn .item-label");n&&(n.textContent=t.isBlocked?"解除拉黑":"加入黑名单")}function eE(e){let t=document.getElementById("diary-entry-view"),a=document.getElementById("diary-entry-title-h1"),i=document.getElementById("diary-entry-content-div"),n=document.getElementById("diary-entry-meta-p"),l=document.getElementById("diary-header-avatar");if(a.textContent="",i.innerHTML="",n.innerHTML="",l.style.display="none",null===e){a.textContent="空空如也",i.innerHTML="<p>这里还没有任何日记。点击右上角的“+”号来写下第一篇吧！</p>",t.dataset.viewingIndex="-1";let r=t.dataset.ownerId;r&&(l.src=ec(r),l.style.display="block"),eI();return}if(w&&"number"==typeof e&&w.logEntries[e]){let o=w.logEntries[e];if("DIARY_ENTRY"===o.key){let s=o.data;if(a.textContent=s.title||"无标题日记",n.innerHTML=`
        ...
        `;let r=n.querySelector(".recall-notice-text"),o=n.querySelector(".recall-content");r.addEventListener("click",()=>{o.classList.toggle("expanded")}),t?t.replaceWith(n):c.appendChild(n)}function e$(e){if(c.innerHTML="",!w)return;let t=w.logEntries.map((e,t)=>({...e,originalIndex:t})).filter(t=>{let a=t.convoId||t.conversationId||t.content&&t.content.convoId||t.data&&t.data.convoId;return a===e}),a=E.filter(t=>t.conversationId===e).map((e,t)=>({...e,originalIndex:w.logEntries.length+t,timestamp:new Date().toISOString()})),i=[...t,...a];i.sort((e,t)=>{if(void 0!==e.originalIndex&&void 0!==t.originalIndex)return e.originalIndex-t.originalIndex;let a=e=>e.timestamp?new Date(e.timestamp):e.data&&e.data.timestamp?new Date(e.data.timestamp):e.content&&e.content.timestamp?new Date(e.content.timestamp):null,i=a(e),n=a(t);return i&&n?i-n:0}),i.forEach(e=>{"RECALL_MESSAGE"!==e.key&&function e(t,a){if("RECALL_MESSAGE"===t.key){let i=t.data.target_text,n=t.data.author,r=Array.from(c.querySelectorAll(".message-row")).reverse(),o=r.find(e=>{let t=e.querySelector(".message-bubble");if(!t)return!1;let a=e.querySelector(".message-avatar").dataset.senderId;return a===n&&t.textContent.trim()===i});o&&e9(t,o,a);return}switch(t.type){case"event_log":e8(t.content,a);break;case"group_event":ek(t.content,a);break;case"time":break;case void 0:t.key||console.warn("Undefined entry type, skipping UI add:",t);break;default:(function e(t,a){let{id:i,sender:n,type:r,data:o}=t,s="user"===n||"{{user}}"===n||n===A.name?"me":"them",d=t.convoId||t.conversationId,m=C.find(e=>e.id===d),p=document.createElement("div");p.className="message-row "+s,void 0!==a&&(p.dataset.logIndex=a),p.dataset.messageId=i;let u=m&&"vgroup"===m.type&&!m.members.includes(n),g="me"===s?"user":n,y;if(u){if(U.has(n))y=U.get(n);else{let f=new Set(U.values()),b=X.filter(e=>!f.has(e)),v=b.length>0?b[Math.floor(Math.random()*b.length)]:X[Math.floor(Math.random()*X.length)];U.set(n,v),y=v}}else y=ec(g);let x=`<img src="${y}" class="message-avatar" data-sender-id="${g}">`,$=document.createElement("div");if($.className="message-content-wrapper",m&&("group"===m.type||"vgroup"===m.type)&&"them"===s){let L=document.createElement("div");L.className="sender-name-label",L.textContent=u?n:ed(n,d),$.appendChild(L)}let B="",D="message-bubble";switch(t.isFailed&&(D+=" failed-message-bubble"),r){case"sticker":let M=function e(t){let a=[];a.push(...ea,...JSON.parse(localStorage.getItem(et)||"[]")),_.forEach(e=>{let t=JSON.parse(localStorage.getItem(ei(e.id))||"[]");a.push(...t)}),C.forEach(e=>{if("group"===e.type){let t=JSON.parse(localStorage.getItem(ei(e.id))||"[]");a.push(...t)}else if("single"===e.type){let i=e.members.find(e=>"user"!==e);if(i){let n=JSON.parse(localStorage.getItem(ei(i))||"[]");a.push(...n)}}});let i=a.filter((e,t,a)=>e.label&&t===a.findIndex(t=>t.label===e.label)),n=i.find(e=>e.label===t);return n?n.url:void 0}(t.content);M?(B=`<img src="${M}" alt="${t.content}">`,D+=" sticker-bubble"):B=`[表情: ${t.content}]`;break;case"forward":{let O=t.data,P="转发的内容",H=O.messageIds,R=!1;if(H&&H.length>0){let q=H[0];if(w.logEntries.some(e=>"MOMENT"===e.key&&e.data.momentId===q))R=!0,P="转发的动态";else{let G=!1;try{let W=JSON.parse(q);W&&"weibo_post"===W.type&&(G=!0)}catch(Y){}P=G?"转发的帖子":"转发的聊天记录"}}let j="",z=null;if(H&&H.length>0){let F=H[0];if(R){let V=w.logEntries.find(e=>"MOMENT"===e.key&&e.data.momentId===F);if(V){let K=V.data,J=ed(K.author,null),Z=K.text||"[图片动态]";j=`<p style="display: flex; align-items: center; gap: 5px;"><i class="far fa-images" style="color: #4CAF50;"></i><strong>@${J}</strong></p><p>${Z.substring(0,30)}</p>`}else j=`<p>一条已失效的朋友圈动态</p>`}else{let Q=!1;try{let ee=JSON.parse(F);ee&&"weibo_post"===ee.type&&(Q=!0,z=ee.postId,j=`<p style="display: flex; align-items: center; gap: 5px;"><i class="fab fa-weibo" style="color: #E14438;"></i><strong>@${ee.author}</strong></p><p>${ee.title||ee.summary}</p>`)}catch(en){}if(!Q){let el=O.messageIds.map(e=>w.logEntries.find(t=>t.id===e)).filter(Boolean);el.slice(0,2).forEach(e=>{let t=`[${e.type||"复合消息"}]`;e.content&&("string"==typeof e.content?t=e.content:e.content.text&&(t=e.content.text)),j+=`<p>${ed(e.senderId||e.sender,null)}: ${t.substring(0,20)}</p>`})}}}let er=z?`data-post-id="${z}"`:"";B=`<div class="forward-card" data-forward-id="${i}" data-message-ids='${JSON.stringify(O.messageIds)}' ${er}>
							<div class="forward-title">${P}</div>
							<div class="forward-summary">${j}</div>
						</div>`,D+=" forward-bubble";break}case"image":if(t.content&&"url"===t.content.type){let eo=t.content.value;if(eo.startsWith("blmx-img-")){let es=sessionStorage.getItem(eo);eo=es||"https://files.catbox.moe/bialj8.jpeg"}B=`<img src="${eo}" alt="图片">`,D+=" image-url-bubble"}else{let em=t.content&&t.content.value?t.content.value:t.content,eu=l(t.id,em);eu?(B=eu,eu.includes("<img")?D+=" sticker-bubble":D+=" image-desc-bubble"):(B=`<div class="image-desc-content">${String(em).replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div>`,D+=" image-desc-bubble")}break;case"location":B=`<div class="location-card"><div class="location-content"><div class="location-title">${t.content}</div><div class="location-subtitle">共享实时位置</div></div><div class="location-map-placeholder"></div></div>`,D+=" location-bubble";break;case"transfer":{let eg="string"==typeof o?JSON.parse(o):o,ey="sent"!==eg.status,ef="";m&&"group"===m.type&&eg.recipientId&&(ef=`<div class="recipient-note">转账给：${ed(eg.recipientId,d)}</div>`);let eh=ey?`<div class="status-text">${"accepted"===eg.status?"已接收":"已退还"}</div>`:`${ef}<div class="note">${eg.note||" "}</div>`;B=`<div class="transfer-card ${ey?"transfer-receipt":"transfer-initial"}"><div class="transfer-content"><img src="https://files.catbox.moe/y8059q.png" class="transfer-icon-image"><div class="transfer-details"><div class="amount">\xa5${eg.amount}</div>${eh}</div></div><div class="transfer-footer">转账</div></div>`,D+=" transfer-bubble";break}case"file":B=`<div class="file-card"><div class="file-content"><i class="fas fa-file-alt file-icon"></i><div class="file-details"><div class="file-name">${t.content.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div></div></div><div class="file-footer">文件</div></div>`,D+=" file-bubble";break;case"gift":{let eb="string"==typeof o?JSON.parse(o):o,ev="";m&&"group"===m.type&&eb.recipientId&&(ev=`<div class="recipient-note">送给：${ed(eb.recipientId,d)}</div>`);let ew="";B=`<div class="gift-content"><i class="fas fa-gift gift-icon"></i><div class="gift-details"><div class="gift-name">${eb.name}</div>${ew="sent"===eb.status?`${ev}`+(eb.price?`<div class="gift-price">\xa5 ${eb.price}</div>`:""):`<div class="gift-status-text">${"accepted"===eb.status?"已收下":"已拒收"}</div>`}</div></div><div class="gift-footer">礼物</div>`,D+=" gift-bubble","sent"!==eb.status&&(D+=" gift-receipt");break}case"music_share":{let eE=t.data||{},eI=eE.cover;eI&&"https://files.catbox.moe/g3x1v8.jpg"!==eI||(eI=ec(t.sender)),B=`
							<div class="music-share-card" data-song-info='${JSON.stringify(eE)}'>
								<div class="music-share-cover" style="background-image: url('${eI}');"></div>
								<div class="music-share-info">
									<div class="music-share-title">${eE.title||"未知歌曲"}</div>
									<div class="music-share-artist">${eE.artist||"未知歌手"}</div>
								</div>
							</div>
							<div class="music-share-footer">
								<i class="fas fa-music"></i> 一起听
							</div>
						`,D+=" music-share-bubble";break}case"payment_receipt":let e9={};try{"object"==typeof t.content&&null!==t.content?e9=t.content:"string"==typeof t.content&&(e9=JSON.parse(t.content))}catch(e8){console.error("代付数据解析失败",e8),e9={total:"0.00",payer:"未知",receiver:"未知"}}let eL="",ek=e9.items||[];ek.slice(0,3).forEach(e=>{eL+=`
            ...
            </div>`}),ek.length>3&&(eL+=`<div class="receipt-row" style="color:#bbb; font-size:0.8em;">... 等共 ${ek.length} 件商品</div>`);let eS=e9.timestamp?e9.timestamp:eT(t.timestamp);B=`
        <div class="payment-receipt-card">
            ...
            <div class="receipt-body">
                ...
                <div class="receipt-amount">\xa5${e9.total||"0.00"}</div>
                <div class="receipt-divider"></div>
                <div class="receipt-row highlight">
                    <span>付款人</span>
                    <span>${e9.payer||"未知"}</span>
                </div>
                <div class="receipt-row">
                    <span>接收人</span>
                    <span>${ed(e9.receiver||"user",null)}</span>
                </div>
                <div class="receipt-divider"></div>
    ...
    `,D="message-bubble shopping-receipt-bubble";break;case"product_share":let eD=t.content,e_=eD.image;!e_||e_.startsWith("http")||e_.startsWith("blob:")||(e_="https://files.catbox.moe/c41va3.jpg"),B=`
                    <div class="product-share-card" data-product='${JSON.stringify(eD)}'>
                        <div class="ps-content">
                            <img src="${e_}" class="ps-img">
                            <div class="ps-info">
                                <div class="ps-title">${eD.title||"未知商品"}</div>
                                <div class="ps-price">\xa5${eD.price||"0.00"}</div>
                            </div>
                        </div>
                        <div class="ps-footer">
                ...
                `,D+=" product-share-bubble";break;case"red_packet":{let eC=t.content,eM=eC.note||"恭喜发财，大吉大利";B=`
<div class="red-packet-card">
	<div class="red-packet-content">
		<i class="fas fa-wallet red-packet-icon"></i>
		<div class="red-packet-details">
			<div class="red-packet-title">${eM}</div>
		</div>
	</div>
	<div class="red-packet-footer">微信红包</div>
</div>`,D+=" red-packet-bubble";break}case"voice":B=`
					...
					`,D+=" voice-bubble";break;default:let eO=t.content.match(/^\[引用:\s*"(.*?):\s*(.*?)"\]\s*(.*)$/s);if(eO){let eN=eO[1],e0=eO[2],eP=eO[3],eH=e0.match(/^\[语音:\s*(.*)\]$/);if(eH)try{let eR=eH[1].replace(/\\"/g,'"'),eq=JSON.parse(eR);e0=`语音：${eq.text}`}catch(e3){console.error("解析引用的语音消息失败:",e3,"原始字符串:",eH[1]),e0="[语音]"}else e0.startsWith("[表情:")?e0="[表情]":e0.startsWith("[图片:")?e0="[图片]":e0.startsWith("[位置:")&&(e0="[位置]");B=`
								<div class="quote-container">
									<div class="quote-author">${eN}</div>
									<div class="quote-content">${e0.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div>
								</div>
								<span class="reply-text">${eP.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</span>
							`}else B=t.content.replace(/</g,"&lt;").replace(/>/g,"&gt;")}let eU=document.createElement("div");eU.className=D,eU.innerHTML=B,$.appendChild(eU);let eG=document.createElement("input");eG.type="checkbox",eG.className="forward-checkbox",eG.dataset.messageId=i;let eW=null;if(t.isFailed&&((eW=document.createElement("i")).className="fas fa-exclamation-circle failed-status-icon"),"me"===s?(p.appendChild(eG),eW&&p.appendChild(eW),p.appendChild($),p.insertAdjacentHTML("beforeend",x)):(p.appendChild(eG),p.insertAdjacentHTML("beforeend",x),p.appendChild($),eW&&p.appendChild(eW)),"voice"===r&&eU.addEventListener("click",()=>{let e=eU.querySelector(".voice-text-content");e&&e.classList.toggle("expanded")}),"forward"===r){let eY=eU.querySelector(".forward-card");eY.addEventListener("click",()=>{let e=eY.dataset.postId;if(e){let t=N.posts.find(t=>t.postId===e);t?(ep("weiboDetail",{postId:e,category:t.category}),eA(e)):showDialog({mode:"alert",text:"该微博已被作者删除或链接已失效。"})}else{let a=eY.dataset.messageIds,i=JSON.parse(a),n=!1;try{let l=JSON.parse(i[0]);l&&"weibo_post"===l.type&&(n=!0)}catch(r){}if(n)showDialog({mode:"alert",text:"这是一个微博转发预览，但由于格式原因无法直接跳转。"});else if(i[0].startsWith("moment_"))(function e(t){let a=document.getElementById("forward-content-modal"),i=document.getElementById("forward-content-modal-body"),n=a.querySelector(".title"),l=parseInt(t.replace("moment_",""),10),r=w.logEntries[l];if(!r||"MOMENT"!==r.key)return;n.textContent="查看动态",i.innerHTML="",eB(null);let o=h.querySelector(`.moment-post[data-post-id="${l}"]`);if(o){let s=o.cloneNode(!0),d=s.querySelector(".post-actions");d&&d.remove(),i.appendChild(s),a.style.display="flex"}eB(S)})(i[0]);else{let o=w.logEntries.find(e=>e.id===eY.dataset.forwardId);o&&function e(t){let a=document.getElementById("forward-content-modal"),i=document.getElementById("forward-content-modal-body"),n=a.querySelector(".title");n.textContent=t.title,i.innerHTML="";let l=t.messageIds.map(e=>w.logEntries.find(t=>t.id===e)).filter(Boolean);l.forEach(e=>{let t=document.createElement("div");t.className="forward-modal-item";let a="object"==typeof e.content&&"voice"!==e.type?`[${e.type||"复合消息"}]`:e.content.text||e.content,n=e.recalled_timestamp||e.data&&e.data.timestamp||e.content&&e.content.timestamp?eT(e.recalled_timestamp||e.data.timestamp||e.content.timestamp):"";t.innerHTML=`
...
`,r.innerHTML="";let p=N.posts.filter(e=>e.author===d.id||c&&"{{user}}"===e.author).sort((e,t)=>new Date(t.timestamp)-new Date(e.timestamp));p.length>0?p.forEach(e=>{let t=document.createElement("div");t.className="post-card",t.dataset.postId=e.postId;let a="",i=e.title||"无标题帖子",n=i.match(/^(?:\[|【)([^\]】]+)(?:\]|】)\s*(.*)$/);if(n){let l=n[1],o=n[2];a=`
				...
				<div class="post-card-footer">
					...
					<div class="post-card-stats" style="display: flex; gap: 0.8rem;">
						...
						<span><i class="far fa-comment-alt"></i> ${(N.comments[e.postId]||[]).length}</span>
		...
		`,o.appendChild(y)}let f=document.createElement("div");if(f.className="ama-qna-list",o.appendChild(f),w){let h=w.logEntries.filter(t=>"AMA_PAIR"===t.key&&(t.data.author===e||c&&"{{user}}"===t.data.author)).sort((e,t)=>new Date(t.data.timestamp)-new Date(e.data.timestamp));if(u&&h.length>0){let b=o.querySelector("p");b&&(b.style.display="none")}h.forEach(e=>{let t=e.data,a=document.createElement("div");a.className="qna-card",a.innerHTML=`
			...
			`,f.appendChild(a)})}s.innerHTML="";let v=[];for(let E in N.comments)N.comments[E].forEach(e=>{(e.author===d.id||c&&"{{user}}"===e.author)&&v.push({...e,post:N.posts.find(e=>e.postId===E)})});v.sort((e,t)=>new Date(t.timestamp)-new Date(e.timestamp)),v.length>0?v.forEach(e=>{if(!e.post)return;let t=e.text,a=t.match(/\[引用:"(?:.*?):\s*(?:[\s\S]*?)"\]([\s\S]*)/);a&&a[1]&&(t=a[1].trim());let i=document.createElement("div");i.className="footprint-item",i.innerHTML=`
			...
			`,s.appendChild(i)}):s.innerHTML='<p style="text-align:center; color: var(--forum-text-secondary); padding: 1rem;">TA还没有发表过任何评论。</p>'}function eA(e){let t=N.posts.find(t=>t.postId===e);if(!t)return;let a=document.getElementById("weibo-detail-view"),i=a.querySelector(".weibo-detail-body"),n=a.querySelector(".weibo-detail-title");i.innerHTML="",J.clear(),n.textContent="";let r=t.author;void 0===t.likes&&(t.likes=Math.floor(1e3*Math.random())),void 0===t.retweets&&(t.retweets=Math.floor(51*Math.random())),void 0===t.bookmarks&&(t.bookmarks=Math.floor(21*Math.random()));let o=(t,a,i=!1,n=!1)=>{let o=e=>{let t=e.replace(/</g,"&lt;").replace(/>/g,"&gt;");return t.replace(/@([^\s@#]+)/g,(e,t)=>`<span class="mention" data-username="${t}">${e}</span>`)},s=t.author,d;if(void 0===t.likes&&(t.likes=Math.floor(10*Math.random())),"user"===s||"{{user}}"===s){let c=K(233);d={...A,name:ed("user"),avatar:ec("user"),postCount:233,title:c.text,titleColor:c.color}}else{let m=_.find(e=>e.id===s);if(m){let p=Math.floor(701*Math.random())+300,u=K(p);d={...m,name:ed(m.id),avatar:ec(m.id),postCount:p,title:u.text,titleColor:u.color}}else d=Z(s)}let g="",y=t.text||t.content||"";if(t.replyTo){let f=(N.comments[e]||[]).find(e=>e.commentId===t.replyTo),h;if(f){let b=ed(f.author,null),v=f.text;h=`<div class="forum-quote-block"><div class="quote-author">@${b}</div><div class="quote-content">${v.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div></div>`}else h=`<div class="forum-quote-block"><div class="quote-content">[原消息已删除]</div></div>`;let w=o(y);g=h+w}else{let E=y.match(/\[引用:"(.*?):\s*([\s\S]*?)"\]([\s\S]*)/);if(E){let I=E[1].trim(),x=E[2].trim(),$=E[3].trim(),L=`<div class="forum-quote-block"><div class="quote-author">${o("@"+I)}</div><div class="quote-content">${x.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div></div>`,k=o($);g=L+k}else g=o(y)}let T="";if(t.image&&t.image_type){let S="width: 100%; height: 200px; object-fit: cover; border-radius: 8px; margin-top: 0.8rem; display: block;";if("url"===t.image_type)T=`<img src="${t.image}" class="post-media-image" style="${S}">`;else if("desc"===t.image_type){let B=l(t.commentId||t.postId,t.image);T=B?`<div style="${S} overflow: hidden; position: relative;">${B}</div>`:`<div class="forum-image-description">${t.image.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div>`}}let D=document.createElement("div");D.className=i?"forum-post-card is-op":n?"forum-post-card is-image-attachment":"forum-post-card",D.dataset.commentId=t.commentId||"post-op",D.dataset.authorName=d.name;let C=!1===t.isRead?'<div class="unread-indicator"></div>':"";if(i){let M=`
				...
				</div>`,O=t.title||"",P=O.match(/^\[([^\]]+)\]\s*(.*)$/);P&&(O=P[2]);let H=`
			...
			`,R="",q=/#([^#\s]+)/g,U=[...(t.text||"").matchAll(q)].map(e=>e[1]);if(U.length>0){let G=[...new Set(U)];R=`
				...
				`,g=g.replace(q,"").trim()}let W=(N.comments[e]||[]).length,Y=`
				...
				</div>`;D.innerHTML=M+H+Y}else{let j=t.author.endsWith("BOT");if(j){let z=document.createElement("div");return z.className="forum-bot-reply",z.innerHTML=`
						<i class="fas fa-robot bot-icon"></i>
						<div class="bot-content-wrapper">
...
`}});let p=t+a+n+r+s+c;return p}();O=e;let t=await v({user_input:e,should_stream:!1});M=t;let a="AI未能生成有效的总结。历史记录未被清除。",i=!1;try{let n=JSON.parse(t.trim());n.summary&&"string"==typeof n.summary&&n.summary.trim()&&(a=`【历史存档摘要】:
...
${t.trim()}`}if(i){let r=w.messageId;await window.parent.TavernHelper.setChatMessage(a,r,{refresh:"none"}),w.logEntries=[],w.messageId=await window.parent.TavernHelper.getLastMessageId(),e3(),ts(),tc(),d.wechatChat.classList.contains("active")&&e$(T),await showDialog({mode:"alert",text:"历史记录已成功总结并存档！手机性能已恢复。"})}else await showDialog({mode:"alert",text:a})}catch(o){console.error("[Archive] AI summary generation failed:",o),await showDialog({mode:"alert",text:`AI总结失败: ${o.message}`})}finally{b=!1,eX()}}function eW(){return new Promise(e=>{let t=document.getElementById("group-chat-modal"),a=document.getElementById("group-chat-contact-list-container"),i=document.getElementById("group-chat-modal-header"),n=document.getElementById("group-chat-modal-footer");a.innerHTML="",i.querySelector(".title").textContent="选择一个作者",i.querySelector("#group-chat-confirm-btn").style.display="none",n.style.display="none";let l=i=>{let n=a.cloneNode(!1);a.parentNode.replaceChild(n,a),t.style.display="none",e(i)},r=i.querySelector("#group-chat-cancel-btn"),o=r.cloneNode(!0);r.parentNode.replaceChild(o,r),o.onclick=()=>l(null),_.forEach(e=>{let t=document.createElement("div");t.className="group-chat-contact-item",t.style.cursor="pointer",t.innerHTML=`
            ...
            `,t.addEventListener("click",()=>{l(e.id)}),a.appendChild(t)}),t.style.display="flex"})}function eY(e){let t=au(),a=t.find(t=>t.id===e);return a&&a.communityBible?a.communityBible:(console.warn(`[AI Context] Could not find community bible for category: ${e}. Using fallback.`),"【错误】未找到对应的社区说明。请基于通用知识进行创作。")}async function ej(){if(b){await showDialog({mode:"alert",text:"AI正在生成中，请稍后再试。"});return}document.getElementById("weibo-feed-view");let e=document.getElementById("weibo-feed-title"),t=e.textContent,a="",n=Array.from(document.querySelectorAll(".weibo-zone-card")).find(e=>e.querySelector(".zone-title").textContent===t);if(n)a=n.dataset.category;else{await showDialog({mode:"alert",text:"错误：无法确定当前帖子分区。"});return}let l=await af({title:"发布新微博",fields:[{id:"title",label:"帖子标题",type:"text"},{id:"body",label:"帖子正文",type:"textarea"},{id:"image_url",label:"图片链接 (可选, 优先使用)",type:"text"},{id:"image_desc",label:"或 图片描述 (可选)",type:"text"}]});if(null===l)return;let{title:r,body:o,image_url:s,image_desc:d}=l,c="",m="none";if(s&&s.trim()?(m="url",c=s.trim()):d&&d.trim()&&(m="desc",c=d.trim()),!r.trim()&&!o.trim()&&!c.trim()){await showDialog({mode:"alert",text:"标题、正文和图片至少需要填写一项。"});return}let p=await showDialog({mode:"confirm",text:"如何发布这篇帖子？\n\n(确定 = 匿名发布)\n(取消 = 实名发布)"}),u;if(p){let g=tT();if(g&&g.name)u=g.name;else{await showDialog({mode:"alert",text:"请先通过右上角的齿轮设置您的马甲才能匿名发帖。"});return}}else u=A.id;let y={author:u,title:r,text:o,image_type:m,image:c,postId:`weibo_${Date.now()}_${Math.random().toString(36).substr(2,9)}_${A.id}`,category:a,timestamp:new Date(window.currentGameDate).toISOString(),likes:Math.floor(100*Math.random()),retweets:Math.floor(21*Math.random()),bookmarks:Math.floor(11*Math.random())};w.addEntry({key:"WEIBO_POST",data:y}),"desc"===y.image_type&&y.image&&i(y.postId,y.image,"weibo"),await w.persistLogToStorage(),e3(),eD(a),ep("weiboFeed",{category:a,categoryName:t}),await showDialog({mode:"alert",text:`新帖子已成功发布！`})}async function ez(){if(b){await showDialog({mode:"alert",text:"AI正在生成中，请稍后再试。"});return}let e=document.getElementById("diary-entry-view").dataset.ownerId;if(!e){console.error("创建日记失败：无法获取日记主人ID。"),await showDialog({mode:"alert",text:"无法确定为谁写日记，请返回封面页重新进入。"});return}let t=ed(e,null),a=await showDialog({mode:"confirm",text:`是否要探寻一下 ${t} 此刻的心事，并让Ta记录下来？`});a&&(await showDialog({mode:"alert",text:`正在探寻 ${t} 的心事，请稍候...`}),await e7(e))}async function e7(e){if(b){await showDialog({mode:"alert",text:"AI正在生成中，请稍后再试。"});return}console.log(`[日记功能] 开始为 ${e} 创建日记...`),b=!0,eX();let t=function e(t){let a=_.find(e=>e.id===t);if(!a)return console.error(`[日记功能] 无法为 ${t} 创建上下文，角色不存在。`),null;let i=`
...
[你的指令]
现在，请严格遵循以上格式和示例，为【${a}】分区生成初始的帖子列表。`;return n.trim()}(e);if(!t){b=!1,eX();return}O=t;try{let a=await v({user_input:t,should_stream:!1});(M=a.trim())&&await ao(M,{contextCategory:e})}catch(i){console.error("[BLMX Forum] AI initial feed generation failed:",i),await showDialog({mode:"alert",text:`AI生成初始帖子列表失败: ${i.message}`})}finally{b=!1,eX()}}async function ac(e){if(b){console.warn("[BLMX Forum] AI is already generating. Aborting full post generation.");return}let t=N.posts.find(t=>t.postId===e);if(!t)return;console.log(`[BLMX Forum] Triggering full post generation for postId: ${e}`),b=!0,eX();let a=function e(t){let a=N.posts.find(e=>e.postId===t);if(!a)return console.error(`[BLMX Forum] 无法为帖子 ${t} 创建上下文，帖子未找到。`),null;let i=document.querySelector(`.weibo-zone-card[data-category="${a.category}"] .zone-title`).textContent,n=eY(a.category),l="",r="";"fanfic"===a.category?(l="正文若为有情节、有细节的短篇故事或章节请适量长一点，若为CP讨论或社交可以适量短一点。",r=`* **【正文内容核心要求】**:
...
[你的指令]
现在，请沉浸到这个论坛里，严格遵循以上所有规则，开始你的创作。`;return o.trim()}(e);if(!a){b=!1,eX();return}O=a,await showDialog({mode:"alert",text:"AI正在撰写帖子内容和评论，请稍候..."});try{let i=await v({user_input:a,should_stream:!1});(M=i.trim())&&await ao(M,{contextCategory:t.category,contextPostId:e})}catch(n){console.error("[BLMX Forum] AI full post generation failed:",n),await showDialog({mode:"alert",text:`AI生成帖子内容失败: ${n.message}`})}finally{b=!1,eX()}}async function am(e){if(b||!w||!v){console.warn("[BLMX Weibo] AI is already generating or essential functions are missing. Aborting.");return}console.log(`[BLMX Weibo] Triggering AI response for post: ${e}`),b=!0,eX();let t=function e(t){let a=N.posts.find(e=>e.postId===t);if(!a)return null;let i=`
* **【最高优先级：核心禁令】**:
1. **人设严格遵守 (OOC禁令)**: 在扮演任何核心角色 (char) 发表评论前，你 **必须** 深入思考该角色的性格和背景。 **绝对禁止** 做出任何不符合角色人设的行为（OOC）。
2. **用户身份禁令**: 用户的ID是 '{{user}}'。你 **绝对禁止** 以 '{{user}}' 的身份或名义生成任何评论、点赞或任何形式的互动。`,n=eY(a.category),l=`
...
`;let s=N.comments[t]||[];if(s.length>0){let d=s.slice(-5);r+=`* **最新评论** (按时间顺序):
...
`;return n.trim()}(n.id,L);if(!k)throw Error("无法为AI生成有效的上下文。");O=k;let T=await v({user_input:k,should_stream:!1});M=T.trim();let S=M.match(/AMA_ANSWER:({.*})/);if(S){let B=JSON.parse(S[1]),D=B.answer,C={key:"AMA_PAIR",data:{author:n.id,question:L,answer:D,timestamp:new Date().toISOString()}};w.addEntry(C),await w.persistLogToStorage();let N=document.createElement("div");N.className="qna-card",N.innerHTML=`
				...
				`,x.prepend(N),$.value=""}else await showDialog({mode:"alert",text:"AI未能给出有效的内心独白，请稍后再试。"})}catch(P){console.error("[AMA AI Call] Failed:",P),await showDialog({mode:"alert",text:`AI响应失败: ${P.message}`})}finally{b=!1,I.innerHTML='<i class="fas fa-paper-plane"></i>',I.disabled=!1}return}let H=e.target.closest("#view-more-ama-btn");if(H){if(!n)return;let R=await showDialog({mode:"confirm",text:`是否要对 ${i} 进行一次“灵魂叩问”？
...
`}),t.appendChild(i("装饰 (Decorations)",c,!1))}(),document.getElementById("bubble-workshop-view").classList.add("is-locked")}),document.getElementById("bubble-workshop-back-btn").addEventListener("click",()=>{ep("home")}),document.getElementById("app-global-studio").addEventListener("click",()=>{ep("globalDesignStudio"),function e(){let t=document.getElementById("studio-body");if(!t)return;let a=t.querySelector("#color-controls-wrapper");a?a.innerHTML="":((a=document.createElement("div")).id="color-controls-wrapper",t.appendChild(a));let i=getComputedStyle(document.documentElement),n={};for(let l in globalThemeVariableMap){let r=globalThemeVariableMap[l];n[r.group]||(n[r.group]=[]),n[r.group].push({name:l,...r})}globalThemeGroupOrder.forEach(e=>{if(n[e]){let t=document.createElement("div");t.className="studio-color-group";let l=document.createElement("h4");l.textContent=e,t.appendChild(l),n[e].forEach(e=>{let a=i.getPropertyValue(e.name).trim(),n=document.createElement("div");if(n.className="studio-color-item","range"===e.type){let l=parseFloat(a)||e.min;n.innerHTML=`
