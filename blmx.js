...
`],g=`
[System Note: You are roleplaying inside a chat app. The user's ID is '{{user}}'.`;if(i)g+=` You MUST account for ALL conversations listed below that have new messages from '{{user}}'. CRITICAL: ALL direct replies MUST be prefixed with the conversation ID, e.g., '[convo_id] speaker_id: message'.

Active Conversations:
`,e.forEach(e=>{let i=a.find(t=>t.id===e);if(i){let n="Private Chat";if("group"===i.type?n=`Group Chat "${i.name}"`:"vgroup"===i.type&&(n=`Virtual Group Chat "${i.name}"`),"vgroup"===i.type)g+=`- ID: [${i.id}], Type: ${n}, Background Members: "${i.background_members_desc}"
`;else{let l=i.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");g+=`- ID: [${i.id}], Type: ${n}, Participants: ${l}
`}}}),u.push(g+"]\n"),e.forEach(t=>{u.push(`--- Conversation History for [${t}] ---`);let a=this.logEntries.filter(e=>{let a=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return a===t}).slice(-5).map(a=>this._formatEntryForAI(a,t,e)).filter(Boolean);u.push(...a),u.push("--- End of History ---\n")});else{let y=e[0],f=a.find(e=>e.id===y);if(!f)return"";let h=f.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");if("vgroup"===f.type){let b=f.members.filter(e=>"user"!==e);g+=` You are roleplaying in a virtual group chat named "${f.name}".
*   **Core Members Present**: ${b.length>0?b.join(", "):"None"}.
*   **Background Members**: You can freely roleplay any number of background members fitting this description: "${f.background_members_desc}".
...
`}else g+=` You are roleplaying in a single chat. Participants: ${h}. Respond as one of the characters (excluding '{{user}}'). Prefix each line with the speaker's ID.]

`;u.push(g);let v=this.logEntries.filter(e=>{let t=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return t===y}).slice(-5).map(t=>this._formatEntryForAI(t,y,e)).filter(Boolean);u.push(...v)}let w=this.logEntries.filter(e=>"MOMENT"===e.key&&e.data&&e.data.momentId&&!e.data.isPrivate).map(e=>`[MOMENT by ${e.data.author} (ID: ${e.data.momentId}) at ${e.data.timestamp}: "${e.data.text||"(no text)"}"]`).join("\n");w&&u.push("\n--- Recent Public Moments ---\n"+w);let E=`
[System Instruction: Your Turn]
**CRITICAL RULE:** This is a mobile phone simulation. Your response MUST ONLY be action commands or chat messages in the specified formats. ALL narrative descriptions (actions, thoughts, environment) are strictly forbidden. Do NOT repeat any previous messages from the history.`;return u.push(E),u.join("\n")}_formatEntryForAI(e,t,a){if("RECALL_MESSAGE"===e.key)return`[${e.data.author} recalled a message]`;if("forward"===e.type){let i=a.includes(t),n=`[${e.sender} forwarded content titled "${e.data.title}":
