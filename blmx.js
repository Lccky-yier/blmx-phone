...
5.  **[发送丰富消息]** (格式: \`角色ID:内容\`)
    ...
    *   \`角色ID:[文件:文件名.后缀]\`
    *   **[表情包规则]**: ${p}
    *   \`角色ID:[sticker:表情名称]\` (注意：表情名称必须严格来自上方的列表)
...
`],b=`
[System Note: You are roleplaying inside a chat app. The user's ID is '{{user}}'.`;if(i)b+=` You MUST account for ALL conversations listed below that have new messages from '{{user}}'. CRITICAL: ALL direct replies MUST be prefixed with the conversation ID, e.g., '[convo_id] speaker_id: message'.

Active Conversations:
`,e.forEach(e=>{let i=a.find(t=>t.id===e);if(i){let n="Private Chat";if("group"===i.type?n=`Group Chat "${i.name}"`:"vgroup"===i.type&&(n=`Virtual Group Chat "${i.name}"`),"vgroup"===i.type)b+=`- ID: [${i.id}], Type: ${n}, Background Members: "${i.background_members_desc}"
`;else{let l=i.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");b+=`- ID: [${i.id}], Type: ${n}, Participants: ${l}
`}}}),h.push(b+"]\n"),e.forEach(t=>{h.push(`--- Conversation History for [${t}] ---`);let a=this.logEntries.filter(e=>{let a=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return a===t}).slice(-5).map(a=>this._formatEntryForAI(a,t,e)).filter(Boolean);h.push(...a),h.push("--- End of History ---\n")});else{let v=e[0],w=a.find(e=>e.id===v);if(!w)return"";let E=w.members.map(e=>"user"===e?"{{user}}":t.find(t=>t.id===e)?.id||"Unknown").join(", ");if("vgroup"===w.type){let I=w.members.filter(e=>"user"!==e);b+=` You are roleplaying in a virtual group chat named "${w.name}".
*   **Core Members Present**: ${I.length>0?I.join(", "):"None"}.
*   **Background Members**: You can freely roleplay any number of background members fitting this description: "${w.background_members_desc}".
...
`}else b+=` You are roleplaying in a single chat. Participants: ${E}. Respond as one of the characters (excluding '{{user}}'). Prefix each line with the speaker's ID.]

`;h.push(b);let x=this.logEntries.filter(e=>{let t=e.conversationId||e.content&&e.content.convoId||e.data&&e.data.convoId;return t===v}).slice(-5).map(t=>this._formatEntryForAI(t,v,e)).filter(Boolean);h.push(...x)}let $=this.logEntries.filter(e=>"MOMENT"===e.key&&e.data&&e.data.momentId&&!e.data.isPrivate).map(e=>`[MOMENT by ${e.data.author} (ID: ${e.data.momentId}) at ${e.data.timestamp}: "${e.data.text||"(no text)"}"]`).join("\n");$&&h.push("\n--- Recent Public Moments ---\n"+$);let L=`
[System Instruction: Your Turn]
**CRITICAL RULE:** This is a mobile phone simulation. Your response MUST ONLY be action commands or chat messages in the specified formats. ALL narrative descriptions (actions, thoughts, environment) are strictly forbidden. Do NOT repeat any previous messages from the history.`;return h.push(L),h.join("\n")}_formatEntryForAI(e,t,a){if("RECALL_MESSAGE"===e.key)return`[${e.data.author} recalled a message]`;if("forward"===e.type){let i=a.includes(t),n=`[${e.sender} forwarded content titled "${e.data.title}":
