		...
		class BLMX_Protocol {
					...
					try {
						...
						if (chatMatch) {
							...
							const systemCommands = [
								...
								'AMA_PAIR', 'VIDEO_CALL', 'INVITE_MEMBER',
								'FOOTPRINTS', 'GALLERY_UPDATE',
								'HIDDEN_ALBUM_UPDATE',
								'TRASH_BIN_UPDATE',
								'SHOPPING_UPDATE',
								'TAOBAO_HOME' // <--- 【在这里添加】
			...
			getContextForAI(activeConvoIds, contacts, conversations, isGlobalMode) {
				...
				// 检测 NAI 是否开启
				const isNaiEnabled = localStorage.getItem(`blmx_nai_enabled_${this.charId}`) === 'true';
...
**CRITICAL RULE:** This is a mobile phone simulation. Your response MUST ONLY be action commands or chat messages in the specified formats. ALL narrative descriptions (actions, thoughts, environment) are strictly forbidden. Do NOT repeat any previous messages from the history.`;
		...
		const globalThemeVariableMap = {
			...
			'--hub-decorative-image-url': { label: '首页顶部 壁纸', group: '全局壁纸', type: 'imageUrl' },
			'--hub-album-art-bg-image': { label: '相册 壁纸', group: '全局壁纸', type: 'imageUrl' },
			'--forum-banner-image': { label: '论坛首页 壁纸', group: '全局壁纸', type: 'imageUrl' },
			...
			// 系统与事件气泡
			'--event-log-bg': { label: '事件气泡 背景', group: '系统与事件气泡' },
			'--event-log-text-color': { label: '事件气泡 文字', group: '系统与事件气泡' },
			'--event-log-border-radius': { label: '事件气泡 圆角', group: '系统与事件气泡', type: 'range', min: 0, max: 20, unit: 'rem', step: 0.1 },
			'--event-desc-expanded-bg': { label: '展开描述 背景', group: '系统与事件气泡' },
			'--event-desc-expanded-border-color': { label: '展开描述 边框', group: '系统与事件气泡' },
			'--event-desc-expanded-text-color': { label: '展开描述 文字', group: '系统与事件气泡' },
			'--event-desc-expanded-font-size': { label: '展开描述 字号', group: '系统与事件气泡', type: 'range', min: 0.5, max: 1.5, unit: 'em', step: 0.05 },
			...
			'--app-icon-gallery-bg': { label: '相册 图标背景', group: 'App 图标背景' },
			'--app-icon-shopping-bg': { label: '淘宝 图标背景', group: 'App 图标背景' },
			...
			// 论坛通用界面
			'--forum-bg': { label: '论坛底层 背景', group: '论坛通用界面' },
			'--forum-card-bg': { label: '论坛卡片 背景', group: '论坛通用界面' },
			'--forum-divider-color': { label: '论坛分割线/边框', group: '论坛通用界面' },
			'--forum-text-primary': { label: '论坛主要文字', group: '论坛通用界面' },
			'--forum-text-secondary': { label: '论坛次要文字', group: '论坛通用界面' },
			'--forum-username-color': { label: '论坛用户名颜色', group: '论坛通用界面' },
			'--forum-action-button-bg': { label: '论坛按钮 背景', group: '论坛通用界面' },
			'--forum-action-button-text': { label: '论坛按钮 文字', group: '论坛通用界面' },
			'--forum-text-quote': { label: '论坛引用文字', group: '论坛通用界面' },
			'--forum-quote-bg': { label: '论坛引用背景', group: '论坛通用界面' },
			'--forum-quote-border': { label: '论坛引用边框', group: '论坛通用界面' },
			'--forum-level-tag-bg': { label: '论坛等级标签背景', group: '论坛通用界面' },
			'--forum-level-tag-text': { label: '论坛等级标签文字', group: '论坛通用界面' },
			'--forum-tag-text': { label: '论坛标签文字', group: '论坛通用界面' },
			'--forum-thread-hover-bg': { label: '论坛帖子悬停背景', group: '论坛通用界面' },
		...
		document.addEventListener('DOMContentLoaded', function() {
			 ...
			 */
			// 在代码中搜索这个函数名，修改它的参数行和缓存逻辑
			function processEntryWithNAI(id, prompt, type, aiPrompt = null) {
				...
				console.log(`[NAI] Request queued for ${id}. Has AI Prompt: ${!!aiPrompt}`);
				
				runtimeImageCache.set(id, "queued");
				refreshActiveUI(type);
				
				// 将 aiPrompt 一起存入队列任务中
				naiRequestQueue.push({ id, prompt, type, aiPrompt });
				processNaiQueue();
			}
			...
			async function processNaiQueue() {
				...
				try {
					...
					// 1. 确定基础描述 (优先用 AI 生成的英文 ai_prompt)
					if (task.aiPrompt && task.aiPrompt.trim() !== "") {
						finalBasePrompt = task.aiPrompt;
						console.log(`[NAI] Using AI-generated prompt for ${task.id}`);
					} else {
						// 兜底逻辑：如果没有 ai_prompt，则翻译中文描述
						finalBasePrompt = await translateToEnglish(task.prompt);
						console.log(`[NAI] Fallback: Translated prompt for ${task.id}`);
					}
					
					// 2. 获取全局正面词
					const userPositive = localStorage.getItem(`blmx_nai_positive_${currentCharId}`) || "";
					
					// 3. 处理外貌注入 (【核心逻辑】仅限视频通话 'call' 类型)
					let charAppearance = "";
					if (task.type === 'call') {
						// 视频通话时，callPartner.id 存储了当前对话的对象
						if (callPartner && callPartner.id) {
							const contact = contacts.find(c => c.id === callPartner.id);
							if (contact && contact.nai_appearance) {
								// 外貌 Tag 通常是存好的英文，如果有中文 translateToEnglish 也会处理
								charAppearance = await translateToEnglish(contact.nai_appearance);
								console.log(`[NAI Call] Injecting appearance for ${callPartner.id}`);
							}
						}
					}
					
					// 4. 最终拼接：[全局正面] + [外貌(仅视频)] + [AI描述/Tag]
					// 注意：微信图片(chat)、朋友圈(moment)、论坛(weibo) 均不进入此 if，只保留正面词和描述
					let finalPromptParts = [userPositive];
					if (charAppearance) finalPromptParts.push(charAppearance);
					finalPromptParts.push(finalBasePrompt);
					
					// 过滤空值并用逗号连接
					let finalPromptString = finalPromptParts
						.map(s => s.trim())
						.filter(s => s !== "")
						.join(", ");
					
					console.log(`[NAI] Final Prompt: ${finalPromptString}`);
					
					// 5. 调用 NAI 发起请求
					const blobUrl = await generateNAI(finalPromptString);
					
					runtimeImageCache.set(task.id, blobUrl);
					console.log(`[NAI] Success for ${task.id}`);
					refreshActiveUI(task.type);
					
				} catch (error) {
					console.error(`[NAI] Error for ${task.id}:`, error);
			...
			/* vvvvvvvv 新增：视频通话-画面历史记录核心逻辑 vvvvvvvv */
			...
			/* vvvvvvvv 替换：renderCallScreen 函数 (V3 - 图文切换版) vvvvvvvv */
			function renderCallScreen() {
				...
				sharedScreen.style.opacity = '1';
				sharedScreen.style.pointerEvents = 'auto';
				sharedScreen.innerHTML = ''; // 清空
				...
				if (data.image) {
					...
					imgEl.className = 'nai-generated-image';
					imgEl.style.cssText = 'width: 100%; height: 100%; object-fit: cover; display: block; border-radius: inherit; cursor: pointer;';
					imgEl.onclick = (e) => {
						e.stopPropagation();
						openImageViewer(data.image);
					};
					sharedScreen.appendChild(imgEl);
				}
				...
				if (data.text) {
					...
					textEl.className = 'screen-description';
					// 如果有图，文字层默认隐藏；没图则显示
					textEl.style.display = data.image ? 'none' : 'block';
					textEl.innerHTML = data.text.replace(/\n/g, '<br>');
					sharedScreen.appendChild(textEl);
				}
				...
				if (data.image && data.text) {
					...
					toggleBtn.innerHTML = '<i class="fas fa-font"></i>'; // 初始图标：文
					toggleBtn.title = "切换图片/文字";
					...
					toggleBtn.onclick = (e) => {
						...
						if (imgEl.style.display !== 'none') {
							...
							toggleBtn.innerHTML = '<i class="fas fa-image"></i>'; // 图标变回图
						} else {
							// 切到图片
							textEl.style.display = 'none';
							imgEl.style.display = 'block';
							toggleBtn.innerHTML = '<i class="fas fa-font"></i>'; // 图标变回文
						}
					};
					
					sharedScreen.appendChild(toggleBtn);
				} else if (!data.image && !data.text) {
					sharedScreen.style.display = 'none';
				}
				
				// 更新箭头状态
				if (currentScreenIndex <= 0) prevBtn.classList.add('disabled');
				else prevBtn.classList.remove('disabled');
				
				if (currentScreenIndex >= callScreenHistory.length - 1) nextBtn.classList.add('disabled');
			...
			const PASSERBY_AVATAR_POOL = [
				'https://i.postimg.cc/bwHC3nxR/image-1768988960491.png',
				'https://i.postimg.cc/h4fYcfnK/image-1768988953162.png',
				'https://i.postimg.cc/nzM9NKBg/image-1770614746053.png',
				'https://i.postimg.cc/C1dch6W6/image-1770614749336.png',
				'https://i.postimg.cc/05TVvnZH/image-1768988974558.png',
				'https://i.postimg.cc/wTFR2TtQ/image-1768988977529.png',
				'https://i.postimg.cc/SQM7tYS9/image_1768988649901.png',
				'https://i.postimg.cc/637LKyCj/image_1768988646350.png',
				'https://i.postimg.cc/156nnnsR/image_1768923269518.png',
				'https://i.postimg.cc/NGKrtCnp/image_1768923263860.png',
				'https://i.postimg.cc/qRSb4m4K/image_1767855389118.png',
				'https://i.postimg.cc/HW8vBC9b/image_1767855396466.png',
				'https://i.postimg.cc/cJvhLVTG/image_1768757827476.png',
				'https://i.postimg.cc/0Qn1rqy3/image_1766304007722.png'
			];
			/* ^^^^^^^^^^ 新增代码到此结束 ^^^^^^^^^^ */
			...
			const defaultGlobalStickers = [{
				label: "好的",
				url: "https://i.postimg.cc/PrVNjD48/image_1766015756837.png"
			}];
			...
			function getAvatar(id) {
				if (id === 'user' || id === '{{user}}') {
					return userProfile.avatar || 'https://i.postimg.cc/jjXTsvCZ/bialj8.jpg';
				}
				const contact = contacts.find(c => c.id === id);
				return (contact && contact.avatar) ? contact.avatar : 'https://i.postimg.cc/jjXTsvCZ/bialj8.jpg';
			}
...
`;
			...
			function getInCallContextForAI(partnerId, history, currentScene) {
				const isNaiEnabled = localStorage.getItem(`blmx_nai_enabled_${currentCharId}`) === 'true';
    ...
    *   **[绘图指令增强 - 必须严格遵守]**:
        ...
        5. **核心逻辑**: 请根据你输出的中文描述，将其深度转化为详细的英文 NAI 格式提示词。` : "";
...
`;
			...
			/* vvvvvvvv 替换：handleUpdateCallScreen 函数 (V6 - 保留文字版) vvvvvvvv */
			async function handleUpdateCallScreen(data) {
				if (!data.type || !data.value) return;
				
				// 1. 立即响应：先存文字和基础状态到历史记录
				addScreenToHistory(data.type, data.value);
				
				const targetIndex = callScreenHistory.length - 1;
				
				// --- NAI 生图逻辑 ---
				const isNaiEnabled = localStorage.getItem(`blmx_nai_enabled_${currentCharId}`) === 'true';
				if (data.type !== 'description' || !isNaiEnabled || !callPartner.id) return;
				
				const contact = contacts.find(c => c.id === callPartner.id);
				const baseAppearance = (contact && contact.nai_appearance) ? contact.nai_appearance : "";
				
				try {
					console.log(`[Call NAI] Processing frame. Has AI Prompt: ${!!data.ai_prompt}`);
					
					let finalActionPrompt = "";
					
					// --- 核心修改：优先使用 AI 提供的 prompt ---
					if (data.ai_prompt && data.ai_prompt.trim() !== "") {
						finalActionPrompt = data.ai_prompt;
					} else {
						// 兜底：翻译描述文字
						finalActionPrompt = await translateToEnglish(data.value);
					}
					
					const translatedBase = await translateToEnglish(baseAppearance);
					const userPositive = localStorage.getItem(`blmx_nai_positive_${currentCharId}`) || "";
					
					// 拼接：[用户全局正面词] + [角色固定外貌描述] + [当前动作描述]
					const parts = [userPositive, translatedBase, finalActionPrompt];
					const finalPrompt = parts.filter(s => s && s.trim() !== "").join(", ");
					
					// 调用生成接口
					const blobUrl = await generateNAI(finalPrompt);
					
					// 7. 回填图片到历史记录中
					if (callScreenHistory[targetIndex]) {
						callScreenHistory[targetIndex].image = blobUrl;
						
						// 如果用户当前正看着这一帧，立即刷新画面
						if (currentScreenIndex === targetIndex) {
							renderCallScreen();
						}
					}
					
				} catch (error) {
					console.error("[Call NAI] Generation failed:", error);
				}
			}
...
`;
			...
			/* vvvvvvvv 替换：handleAcceptCall 函数 (V4 - 修复接听背景与死循环) vvvvvvvv */
			/**
			 * (已更新 V4) 处理用户点击“接听”按钮的逻辑。
			 * 修复：手动设置背景图，防止界面空白；加强错误处理防止灵动岛卡死。
			 */
			async function handleAcceptCall() {
				...
				// 3. 【核心修复】获取DOM并手动设置背景！
				const inCallScreen = document.getElementById('in-call-screen');
				const headerName = inCallScreen.querySelector('.caller-name');
				
				// 确保有名字和背景
				headerName.textContent = callPartner.name;
				// --- [修复点] 手动设置大背景为对方头像 ---
				inCallScreen.style.backgroundImage = `url('${callPartner.avatar}')`;
				
				// 4. 初始化历史记录
				// 第一帧显示对方头像，作为共享画面的默认状态
				callScreenHistory = [];
				currentScreenIndex = -1;
				addScreenToHistory('image', callPartner.avatar);
				...
				isGenerating = true;
				updateFooterButtonsState();
				if (typeof updateInCallButtonUI === 'function') updateInCallButtonUI();
				...
				try {
					...
					console.error('[Call Accept] AI failed to respond:', e);
					// 如果出错，给个提示
					const log = document.getElementById('chat-simulation-log');
					const errEl = document.createElement('div');
					errEl.className = 'chat-simulation-message';
					errEl.style.color = '#ff9999';
					errEl.textContent = '(信号连接不稳定...)';
					log.insertBefore(errEl, log.firstChild);
				} finally {
					// 9. 【重要】无论成功失败，必须解除锁定，停止灵动岛跳动
					isGenerating = false;
					updateFooterButtonsState();
					if (typeof updateInCallButtonUI === 'function') updateInCallButtonUI();
				}
			...
			function addMessageToWeChat(entry, index) {
				...
				switch (type) {
					...
					case 'forward':
					{
							...
							// 检查是否是微博
								...
								try {
									...
									if (potentialJson && potentialJson.type === 'weibo_post') {
										isWeiboPost = true;
										if (!fwdTitle) fwdTitle = '转发的帖子';
									}
								} catch (e) { /* Not JSON */ }
							}
						}
						
						// 如果上面都没匹配到，且没有标题，默认为聊天记录
						if (!fwdTitle) fwdTitle = '转发的聊天记录';
						
						let summaryHtml = '';
						let postId = null; // 用于微博跳转
						...
						if (messageIds && messageIds.length > 0) {
							...
							if (isMomentForward) {
								...
								if (momentEntry) {
									...
									// 【情况B：AI捏造的朋友圈 (修复点)】
									// 显示“匿名用户”和“查看详情”提示
									summaryHtml = `<p style="display: flex; align-items: center; gap: 5px;"><i class="far fa-images" style="color: #4CAF50;"></i><strong>@匿名用户</strong></p><p>点击查看内容</p>`;
								}
								...
								if (messagesToDisplay.length > 0) {
									...
									// 如果找不到消息（也是伪造的聊天记录），就显示“查看详情”
									summaryHtml = `<p>[聊天记录] 点击查看详情</p>`;
								}
							}
						}
						...
						// 【核心关键】把 fwdTitle 存入 data-title 属性，方便点击时提取
						bubbleHtml = `<div class="forward-card" data-forward-id="${id}" data-message-ids='${JSON.stringify(fwdData.messageIds)}' ${postIdAttribute} data-title="${fwdTitle.replace(/"/g, '&quot;')}">
					...
					case 'image':
						if (entry.content && entry.content.type === 'url') {
							...
							if (imageUrl.startsWith('blmx-img-')) {
								...
								imageUrl = storedImage || 'https://i.postimg.cc/jjXTsvCZ/bialj8.jpg';
							}
    ...
    `;
					...
					case 'product_share':
						...
						if (prodImgUrl && !prodImgUrl.startsWith('http') && !prodImgUrl.startsWith('blob:')) {
							...
							prodImgUrl = 'https://i.postimg.cc/HxGwksMx/image_1775470321791.png';
						}
...
<div class="red-packet-card">
					...
					card.addEventListener('click', () => {
						...
						if (postId) {
							...
							if (isOldWeiboFormat) {
								...
								const titleFromCard = card.dataset.title || '转发的动态';
								renderForwardedMomentModal(messageIds[0], titleFromCard);
							} else {
    ...
    `;
			...
			function renderMomentsFeed(authorIdToShow) {
				...
				document.getElementById('moments-cover-photo').src = headerAuthor.cover || 'https://i.postimg.cc/jjXTsvCZ/bialj8.jpg';
...
`;
			...
			function appendNewCommentToForum(commentData) {
				...
				// 在 appendNewCommentToForum 函数内找到这个位置
				const createForumCard = (data, level, isOp = false, isImageAttachment = false) => {
					...
					const authorId = data.author;
					let profile;
					// 【核心修复】增加对 userProfile.name 的匹配判定
					if (authorId === 'user' || authorId === '{{user}}' || authorId === userProfile.name) {
						const postCount = 233;
						const titleInfo = getForumTitleByPostCount(postCount);
						profile = {
							...userProfile,
						...
						const knownContact = contacts.find(c => c.id === authorId || c.name === authorId);
						if (knownContact) {
							const postCount = Math.floor(Math.random() * 701) + 300;
							const titleInfo = getForumTitleByPostCount(postCount);
							profile = {
								...knownContact,
								name: getDisplayName(knownContact.id),
								avatar: getAvatar(knownContact.id),
								postCount,
								title: titleInfo.text,
								titleColor: titleInfo.color
							};
			...
			/* vvvvvvvv 替换：论坛个人主页渲染函数 (ICity简约风格版) vvvvvvvv */
			async function renderForumProfile(contactId) {
				...
				const tagEl = view.querySelector('.profile-post-tag'); // 指向新的帖子数量标签容器
				const signatureEl = view.querySelector('.profile-signature');
				const headerSection = view.querySelector('.profile-header-section'); // 背景图容器
				...
				// 1. 确定当前查看的是“用户”还是“特定角色”
				const isOwnProfile = (contactId === 'user' || contactId === '{{user}}');
				const profileData = isOwnProfile ? userProfile : contacts.find(c => c.id === contactId);
				
				if (!profileData) {
					console.error(`[Forum Profile] 渲染失败: 找不到 ID 为 ${contactId} 的联系人数据`);
					return;
				}
				
				// --- 【核心映射逻辑】 ---
				const momentsCover = profileData.cover || 'https://i.postimg.cc/Y2sB0b11/image_1766129556981.png';
				headerSection.style.backgroundImage = `url("${momentsCover}")`;
				
				// 2. 填充基础信息 (头像、名字、标签、签名)
				avatarEl.src = getAvatar(profileData.id);
				nameEl.textContent = '@' + getDisplayName(profileData.id, null);
				
				// 签名逻辑：直接映射朋友圈签名，继承全局字体
				signatureEl.textContent = profileData.signature || '这个人很懒，什么都没留下。';
				
				// 计算发帖总数 (包含 user 别名兼容)
				const postCount = weiboData.posts.filter(p => p.author === profileData.id || (isOwnProfile && (p.author === 'user' || p.author === '{{user}}'))).length;
				tagEl.textContent = `${postCount} 篇帖子`;
				
				// 3. 渲染“帖子”选项卡
				postsTab.innerHTML = '';
				const userPosts = weiboData.posts.filter(p => p.author === profileData.id || (isOwnProfile && (p.author === 'user' || p.author === '{{user}}')))
					.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
                ...
                `;
						...
						// --- 新增：处理预览文字的加粗显示 ---
						let formattedText = postText.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
						// --- 新增结束 ---
                    ...
                    ${hashtags.map(tag => `<span class="hashtag">#${tag}</span>`).join('')}
						...
						const cleanText = formattedText.replace(hashtagRegex, '').trim();
            ...
            `;
					...
					postsTab.innerHTML = '<p style="text-align:center; color: var(--forum-text-secondary); padding: 2rem;">TA还没有发布过任何帖子。</p>';
				}
        ...
        `;
					...
					placeholder.style.cssText = 'text-align:center; color: var(--forum-text-secondary); padding: 2rem;';
					placeholder.textContent = '这里是你的匿名提问箱。';
            ...
            `;
					...
					footprintsTab.innerHTML = '<p style="text-align:center; color: var(--forum-text-secondary); padding: 2rem;">TA还没有发表过任何评论。</p>';
				}
			}
			/* ^^^^^^^^^^ 替换代码到此结束 ^^^^^^^^^^ */
			...
			function renderWeiboDetail(postId) {
				...
				const createForumCard = (data, level, isOp = false, isImageAttachment = false) => {
					...
					// 【核心修复】增加对 userProfile.name 的匹配判定，解决刷新变路人的问题
					if (authorId === 'user' || authorId === '{{user}}' || authorId === userProfile.name) {
						const postCount = 233;
						const titleInfo = getForumTitleByPostCount(postCount);
						profile = {
			...
			/**
			 * 显示通用角色选择弹窗
			 * @param {string} titleText - 弹窗显示的标题
			 * @returns {Promise<string|null>} - 返回被选中角色ID的字符串，如果取消则返回null
			 */
			function showCommonSelectionModal(titleText = "请选择") {
				return new Promise(resolve => {
					const modal = document.getElementById('common-selection-modal');
					const listContainer = modal.querySelector('.common-list-container');
...
`;
				...
				const fullContext = systemPrompt + chatHistory + momentsHistory + weiboHistory + otherEventsHistory;
				return fullContext;
			}
			/* ^^^^^^^^^^ 替换代码到此结束 ^^^^^^^^^^ */
			...
			function handleAiChatMessage(value, senderId, targetConversationId) {
				...
				if (voiceMatch) {
					...
					if (newEntry.content && newEntry.content.type === 'desc' && newEntry.content.value) {
						processEntryWithNAI(newEntry.id, newEntry.content.value, 'chat', newEntry.content.ai_prompt);
						delete newEntry.content.ai_prompt;
					}
				} else if (locationMatch) {
			...
			function renderConversationList() {
				...
				sortedConversations.forEach(convo => {
					...
					if (convo.id === 'moments_feed') {
						avatarSrc = userProfile.cover || 'https://i.postimg.cc/jjXTsvCZ/bialj8.jpg';
						name = '朋友圈';
					} else if (convo.type === 'group') {
						avatarSrc = convo.avatar || 'https://i.postimg.cc/jjXTsvCZ/bialj8.jpg';
						name = `${convo.name} (${convo.members.length})`;
						if (convo.dissolved) name += " (已解散)";
					} else if (convo.type === 'vgroup') {
						avatarSrc = convo.avatar || 'https://i.postimg.cc/T2JVTx3M/image_1775547027188.png';
						name = convo.name;
					} else {
						const otherMemberId = convo.members.find(m => m !== 'user');
						if (otherMemberId) {
							...
							avatarSrc = 'https://i.postimg.cc/jjXTsvCZ/bialj8.jpg';
							name = '未知对话';
						}
					}
...
`;
			...
			async function resetDefaultFont() {
				...
				if (confirmed) {
					...
					document.documentElement.style.setProperty('--custom-font-family', "'Noto Serif CJK'");
			...
			function initNovelAI() {
				...
				// --- 新增代理相关元素 ---
				const proxySelect = document.getElementById('novelai-proxy');
				const proxyCustomInput = document.getElementById('novelai-proxy-custom');
				...
				// 读取代理设置
				const savedProxy = localStorage.getItem(`blmx_nai_proxy_${currentCharId}`) || 'https://corsproxy.io/?';
				...
				// 应用代理到 UI
				const isCustom = !['https://corsproxy.io/?', 'https://api.allorigins.win/raw?url=', 'https://cors-anywhere.herokuapp.com/', 'none'].includes(savedProxy);
				if (isCustom) {
					proxySelect.value = 'custom';
					proxyCustomInput.value = savedProxy;
					proxyCustomInput.style.display = 'block';
				} else {
					proxySelect.value = savedProxy;
					proxyCustomInput.style.display = 'none';
				}
				...
				proxySelect.addEventListener('change', (e) => {
					...
					if (val === 'custom') {
						...
						proxyCustomInput.style.display = 'none';
						localStorage.setItem(`blmx_nai_proxy_${currentCharId}`, val);
					}
				});
				proxyCustomInput.addEventListener('input', (e) => {
					localStorage.setItem(`blmx_nai_proxy_${currentCharId}`, e.target.value.trim());
				});
				...
				console.log('[NovelAI] Initialization complete.');
			}
			...
			async function generateNAI(prompt) {
				...
				// 获取保存的代理
				const proxyPrefix = localStorage.getItem(`blmx_nai_proxy_${currentCharId}`) || 'https://corsproxy.io/?';
				...
				console.log(`[NAI] Requesting via Proxy: ${proxyPrefix}`);
				
				// 3. 发起请求 (动态组装 URL)
				const endpoint = isV4 ? 'generate-image-stream' : 'generate-image';
				const apiEndpoint = `https://image.novelai.net/ai/${endpoint}`;
				
				let finalUrl;
				if (proxyPrefix === 'none') {
					finalUrl = apiEndpoint; // 不使用代理
				} else {
					// 拼接代理前缀和编码后的 API 地址
					finalUrl = proxyPrefix + encodeURIComponent(apiEndpoint);
				}
...
`
			...
			async function recoverItemToGallery(charId, trashItem, trashImgId) {
				// 1. 构造新照片对象
				const newItem = {
					image: trashItem.image,
					caption: `[已恢复] ${trashItem.text}`,
					time: "刚刚",
					ai_prompt: trashItem.ai_prompt || "" // 保留提示词
				};
				
				// 2. 找到并更新主相册 (gallery_update)
				let latestGallery = [...blmxManager.logEntries].reverse().find(e =>
					e.type === 'gallery_update' && e.author === charId
				);
				let newItemsList = latestGallery ? [...latestGallery.content.items] : [];
				newItemsList.unshift(newItem);
				
				// 3. 【核心修复：删除回收站里的本地储存】
				// 找到该角色最新的回收站记录
				const trashEntry = blmxManager.logEntries.find(e =>
					e.type === 'trash_bin_update' && e.author === charId
				);
				if (trashEntry && trashEntry.content.items) {
					// 从数组中滤掉当前恢复的这一项
					trashEntry.content.items = trashEntry.content.items.filter(i => i !== trashItem);
					// 如果回收站滤完后空了，直接删除这条日志条目，彻底省Token
					if (trashEntry.content.items.length === 0) {
						blmxManager.logEntries = blmxManager.logEntries.filter(e => e !== trashEntry);
					}
				}
				...
				// 5. 【核心修复：图片缓存映射迁移】
				// 将原本属于“回收站ID”的图片地址，复制给“新相册ID”，防止变回文本
				const cachedImage = runtimeImageCache.get(trashImgId);
				if (cachedImage) {
					// 恢复后的照片在 items 数组的第一位，索引是 0
					runtimeImageCache.set(`${newEntryId}_img_0`, cachedImage);
				}
				
				// 6. 保存到酒馆并刷新UI
				await blmxManager.persistLogToStorage();
				await showDialog({ mode: 'alert', text: '照片已恢复并从回收站移除！' });
					...
					shopItems.forEach(item => {
						...
						if (mediaHtml) {
							...
							if (item.image && (item.image.startsWith('http') || item.image.startsWith('blob:'))) {
								mediaHtml = `<img src="${item.image}" style="width:100%; height:100%; object-fit:cover; display:block;" onerror="this.onerror=null;this.src='https://i.postimg.cc/Cx6hDcwn/c41va3.jpg';">`;
							} else {
								// 纯文本描述
								isTextOnly = true;
								const safeText = (item.image || "").replace(/"/g, '&quot;');
								mediaHtml = `<div class="sp-text-placeholder" data-full-text="${safeText}" style="background: url('https://i.postimg.cc/Cx6hDcwn/c41va3.jpg') center/cover no-repeat; color: transparent;">${item.image}</div>`;
							}
						}
...
`;
				...
				if (data.items && data.items.length > 0) {
					data.items.forEach((item, index) => {
						...
						if (!mediaHtml) {
							const safeText = (item.image || "").replace(/"/g, '&quot;');
							mediaHtml = `<div class="tb-img-placeholder-text" data-full-text="${safeText}" style="background: url('https://i.postimg.cc/Cx6hDcwn/c41va3.jpg') center/cover no-repeat;"></div>`;
						}
...
`;
			...
			function setupEventListeners() {
				...
				// --- 沉浸模式开关逻辑 ---
				const immersiveSwitch = document.getElementById('immersive-mode-switch');
				const phoneFrame = document.querySelector('.phone-frame');
				
				// 读取保存的状态
				const isImmersiveStored = localStorage.getItem(`blmx_immersive_mode_${currentCharId}`) === 'true';
				immersiveSwitch.checked = isImmersiveStored;
				
				immersiveSwitch.addEventListener('change', (e) => {
					const isEnabled = e.target.checked;
					localStorage.setItem(`blmx_immersive_mode_${currentCharId}`, isEnabled);
					
					if (isEnabled) {
						phoneFrame.classList.add('immersive-mode');
						document.body.style.height = '100vh';
						document.body.style.overflow = 'hidden';
						document.documentElement.style.overflow = 'hidden';
						window.scrollTo(0, 0);
					} else {
						phoneFrame.classList.remove('immersive-mode');
						// 恢复原始 body 状态
				...
				// 1. 相册直达
				document.getElementById('app-gallery-direct').addEventListener('click', async () => {
					const targetId = await showCommonSelectionModal("查看谁的相册？");
					if (targetId) {
						currentCheckPhoneTargetId = targetId;
						navigateTo('gallery');
						renderGalleryApp(targetId);
					}
				});
				
				// 2. 淘宝直达
				document.getElementById('app-shopping-direct').addEventListener('click', async () => {
					const targetId = await showCommonSelectionModal("查看谁的淘宝？");
					if (targetId) {
						currentCheckPhoneTargetId = targetId;
						navigateTo('shoppingHome');
						renderShoppingHome(targetId);
					}
				});
				...
				document.getElementById('tb-home-back-btn').addEventListener('click', () => {
					navigateTo('home');
				});
				...
				const homeView = document.getElementById('app-homescreen');
				const observer = new MutationObserver(() => {
					if (homeView.classList.contains('active')) {
						// 每次显示主屏幕时，刷新时钟组件的背景为用户最新头像
						const avatarUrl = userProfile.avatar || getAvatar('user');
						document.documentElement.style.setProperty('--hub-music-bar-bg-image', `url('${avatarUrl}')`);
					}
				});
...
`;
				...
				// --- [新增] 设置角色外观 (NAI) ---
				document.getElementById('set-char-appearance-btn').addEventListener('click', async () => {
					// 1. 获取当前联系人 ID
					const contactId = document.getElementById('contact-details-profile-card').dataset.contactId;
					const contact = contacts.find(c => c.id === contactId);
					if (!contact) return;
					...
					// 2. 弹出输入框
					const currentAppearance = contact.nai_appearance || '';
					const newAppearance = await showDialog({
						mode: 'prompt',
						text: `【设置 ${getDisplayName(contact.id, null)} 的外貌特征】\n\n请填写固定的外貌Tag (英文最佳，中文会自动翻译)。\nAI生成视频画面时会强制使用这些特征。\n\n示例: 1girl, silver hair, purple eyes, maid outfit`,
						defaultValue: currentAppearance
					});
					
					// 3. 保存
					if (newAppearance !== null) {
						contact.nai_appearance = newAppearance.trim();
						saveData(); // 保存到本地存储
						await showDialog({ mode: 'alert', text: '外貌设定已保存！下次视频通话时生效。' });
					}
				});
			...
			async function parseAndHandleAiResponse(rawResponse, context = {}) {
				...
				const processCommandData = (key, jsonString) => {
					try {
						...
						switch (key) {
							case 'WEIBO_POST':
								// 找到 case 'WEIBO_POST' 内部：
								if (data.image_type === 'desc' && data.image) {
									processEntryWithNAI(data.postId, data.image, 'weibo', data.ai_prompt); // 增加参数
							...
							case 'GALLERY_UPDATE':
								...
								if (data.items && Array.isArray(data.items)) {
									data.items.forEach((item, index) => {
										...
										// 示例：在 data.items.forEach 循环里
										if (item.image) {
											processEntryWithNAI(uniqueImgId, item.image, 'gallery', item.ai_prompt); // 增加参数
										}
									});
								}
								const galleryView = document.getElementById('cp-gallery-view');
							...
							case 'MOMENT':
								data.momentId = `moment_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
								// 找到 case 'MOMENT' 内部的这一行：
								if (data.image_type === 'desc' && data.image) {
									processEntryWithNAI(data.momentId, data.image, 'moment', data.ai_prompt); // 增加参数
									delete data.ai_prompt;
								}
								blmxManager.addEntry({ key, data });
								if (data.author !== 'user') {
...
* **正文 (必须有1条)**:
* **格式**: \`WEIBO_POST_CONTENT:{"content":"正文内容...使用 \`\\n\` 分段","image":"图片画面简述","image_type":"desc"${isNaiEnabled ? ', "ai_prompt":"Detailed English NAI tags"' : ''}}\`
...
`;
				...
				return new Promise(resolve => {
					...
					options.fields.forEach(field => {
						...
						if (field.type === 'switch') {
							...
							// --- 找到 showMultiInputDialog 内部处理普通输入框/文本域的逻辑部分并替换 ---
						} else {
							...
							let input;
							// 【核心修改】：在这里增加对 textarea-readonly-prefix 的支持
							if (field.type === 'textarea' || field.type === 'textarea-readonly-prefix') {
								input = document.createElement('textarea');
								input.rows = 5; // 设置为5行高度，解决太窄的问题
								input.style.resize = 'vertical';
								input.style.width = '100%'; // 确保宽度撑满
								...
								input = document.createElement('input');
								input.type = 'text';
								input.style.width = '100%'; // 普通输入框也确保撑满
								input.style.boxSizing = 'border-box';
							}
							...
							if (field.type === 'textarea-readonly-prefix' && field.prefix) {
								// 如果初始值里还没带前缀，自动补上
								if (!displayValue.startsWith(field.prefix)) {
			...
			function showForwardTargetModal(data, mode) {
				...
				// --- 核心修复：补充由于更新遗失的转发列表渲染逻辑 ---
				if (mode === 'forward') {
					modal.dataset.messageIds = JSON.stringify(data);
					document.getElementById('group-chat-modal-title').textContent = "选择一个聊天转发";
					conversations.forEach(convo => {
						if (convo.id === 'moments_feed' || convo.type === 'system' || convo.dissolved) return;
						let name, avatarSrc;
						if (convo.type === 'group') {
							name = `${convo.name} (${convo.members.length})`;
							avatarSrc = convo.avatar || 'https://i.postimg.cc/jjXTsvCZ/bialj8.jpg';
			...
			/* vvvvvvvv 替换：统一的弹窗“确定”按钮处理器 (V6) vvvvvvvv */
			...
			/* vvvvvvvv 替换：朋友圈转发查看函数 (支持渲染 AI 捏造的假动态) vvvvvvvv */
			function renderForwardedMomentModal(momentId, fallbackText) {
				const modal = document.getElementById('forward-content-modal');
				const body = document.getElementById('forward-content-modal-body');
				const title = modal.querySelector('.title');
				...
				// === [核心修改] 分支 1：如果是 AI 捏造的假 ID (找不到记录) ===
				if (!originalMoment || originalMoment.key !== 'MOMENT') {
					// 手搓一个假的动态卡片，只有名字和内容
					body.innerHTML = `
...
`;
				...
				// === 分支 2：如果是真实存在的动态 (原有逻辑) ===
				// 先渲染一遍列表以生成 DOM (为了复用样式和结构)
				renderMomentsFeed(null);
				const originalPostElement = document.getElementById('moments-feed-list').querySelector(`.moment-post[data-moment-id="${originalMoment.data.momentId}"]`);
				
				if (originalPostElement) {
					const clonedPost = originalPostElement.cloneNode(true);
					// 移除操作栏，防止在弹窗里点赞
					const actionButtons = clonedPost.querySelector('.post-actions-top');
					if (actionButtons) actionButtons.style.display = 'none';
					const replyBtn = clonedPost.querySelector('.moment-reply-btn');
		...
		document.addEventListener('DOMContentLoaded', () => {
			...
			if (fullscreenTrigger && phoneFrame) {
				...
				document.addEventListener('fullscreenchange', () => {
					const isImmersiveEnabled = localStorage.getItem(`blmx_immersive_mode_${currentCharId}`) === 'true';
