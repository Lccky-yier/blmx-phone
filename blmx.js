...
<div class="red-packet-card">
	...
	<div class="red-packet-footer">微信红包</div>
					...
					`,k+=" voice-bubble";break;default:let e_=t.content.match(/^\[引用:\s*"(.*?):\s*(.*?)"\]\s*(.*)$/s);if(e_){let eD=e_[1],eC=e_[2],eA=e_[3],eM=eC.match(/^\[语音:\s*(.*)\]$/);if(eM)try{let eN=eM[1].replace(/\\"/g,'"'),e0=JSON.parse(eN);eC=`语音：${e0.text}`}catch(eP){console.error("解析引用的语音消息失败:",eP,"原始字符串:",eM[1]),eC="[语音]"}else eC.startsWith("[表情:")?eC="[表情]":eC.startsWith("[图片:")?eC="[图片]":eC.startsWith("[位置:")&&(eC="[位置]");L=`
