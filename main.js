				...
				document.querySelector('.check-phone-grid').addEventListener('click', (e) => {
					...
					if (item) {
						...
						if (appType === 'footprints') {
							...
							navigateTo('shopping');
							renderShoppingApp(charId); // <--- 改为调用渲染函数
						}
						else {
							// 其他未开发的 APP 继续弹窗
			...
			async function parseAndHandleAiResponse(rawResponse, context = {}) {
				...
				for (const line of lines) {
					...
					if (commandMatch) {
						...
						// 情况B：匹配到了聊天格式，检查“内容”是否其实是指令
						const convoId = chatMatch[1];
						const senderId = chatMatch[2].trim();
						const content = chatMatch[3].trim();
						
						// 定义一个检测“嵌入式指令”的正则
						const embeddedCommandRegex = new RegExp(`^(${commandKeywords.join('|')}):\\s*(\\{.*\\})$`);
						const embeddedMatch = content.match(embeddedCommandRegex);
						
						if (embeddedMatch) {
							console.log(`[BLMX Parser] 拦截到带有前缀的指令: ${embeddedMatch[1]} (原发送者: ${senderId})`);
						...
						if (rescueMatch) {
							console.log(`[BLMX Parser] 触发救急匹配指令: ${rescueMatch[1]}`);
